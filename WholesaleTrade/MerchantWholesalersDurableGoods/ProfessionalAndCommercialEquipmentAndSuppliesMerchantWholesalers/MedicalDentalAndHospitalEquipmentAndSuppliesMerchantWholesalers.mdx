...
| DrugRegulator | Regulatory oversight agency |
...
| Recall | Regulatory recall notification |
...
| processRecall | Handle regulatory recall notification |
...
| recallIssued | Regulator announced product recall |
| recallCompleted | Recall actions finished |
| lotExpirationNeared | Lot approaching expiration |
| calibrationBecameDue | Equipment needs calibration |
| certificationExpirationNeared | Certification needs renewal |
...
```typescript
import { wholesaleMedicalEquipment } from '@headlessly/wholesale-medical-equipment'

const wholesaler = wholesaleMedicalEquipment()
...
wholesaler.lotExpirationNeared(async ({ lotNumber, daysRemaining }) => {
  if (daysRemaining <= 30) {
