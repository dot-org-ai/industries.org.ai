...
| agreementExpirationNeared | Representation contract nearing end |
...
// Search represented manufacturers and suppliers
const findPrincipalsResult = await agency.findPrincipals({ status: 'active', territory: 'northeast' })

// Search customer accounts and prospects
const findBuyersResult = await agency.findBuyers({ status: 'qualified', region: 'northeast' })
...
agency.agreementExpirationNeared(async ({ principalId, expirationDate, daysRemaining }) => {
  if (daysRemaining <= 60) {
