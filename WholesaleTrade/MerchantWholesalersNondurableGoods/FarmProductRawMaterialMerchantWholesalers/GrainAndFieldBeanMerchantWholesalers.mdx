...
| ElevatorOperator | Runs grain legs, conveyors, and drying equipment at the facility |
...
| storageBecameFull | Bin capacity reached |
| hedgePlaced | Futures position established |
| qualityIssueDetected | Grain condition requires attention |
...
// Check grain stocks by type, grade, and location
const getInventoryResult = await elevator.getInventory({ status: 'available', category: 'main' })

// Retrieve open purchase and sale contracts
const getContractsResult = await elevator.getContracts({ status: 'active', department: 'public-works' })
