...
| taxFilingBecameDue | Excise tax deadline approaching |
| taxFiled | Excise report submitted |
| licenseExpirationNeared | Retail customer license nearing expiration |
...
distributor.taxFilingBecameDue(async ({ state, period, dueDate }) => {
...
distributor.licenseExpirationNeared(async ({ accountId, licenseId, daysRemaining }) => {
  if (daysRemaining <= 30) {
    await notifyAccount({
      accountId,
