...
| dateCodeExpiring | Products approaching freshness date |
...
distributor.dateCodeExpiring(async ({ productId, dateCode, daysRemaining }) => {
  if (daysRemaining < 5) {
...
distributor.temperatureExcursion(async ({ loadId, temperature, duration }) => {
  if (duration > 30) {
    await createAlert({
      severity: 'critical',
