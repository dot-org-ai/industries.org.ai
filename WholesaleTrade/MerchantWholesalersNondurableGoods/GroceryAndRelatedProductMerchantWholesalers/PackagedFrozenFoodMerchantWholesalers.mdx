...
| ReeferTruckCoordinator | Manages refrigerated transport fleet and temperature compliance |
...
| expirationNeared | Products near expiry |
...
| seasonalPeakReached | High demand period started |
...
```typescript
import { distributeFrozenFoods } from '@headlessly/distribute-frozen-foods'

const distributor = distributeFrozenFoods()
...
distributor.temperatureExcursionDetected(async ({ loadId, temperature, threshold }) => {
  await createAlert({
    severity: 'high',
...
distributor.expirationNeared(async ({ productId, lotNumber, daysRemaining }) => {
  if (daysRemaining < 30) {
