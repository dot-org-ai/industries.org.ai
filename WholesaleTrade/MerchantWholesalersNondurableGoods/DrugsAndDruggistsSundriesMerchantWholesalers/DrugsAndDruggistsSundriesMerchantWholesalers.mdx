...
| expiryApproaching | Products near expiration |
...
```typescript
import { distributeDrugsDruggists } from '@headlessly/distribute-drugs-druggists'

const distributor = distributeDrugsDruggists()
...
distributor.expiryApproaching(async ({ ndc, lotNumber, daysRemaining }) => {
  if (daysRemaining < 90) {
