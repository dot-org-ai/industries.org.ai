...
| backToSchoolStarted | School season approaching |
...
```typescript
import { distributeOfficeSupplies } from '@headlessly/distribute-office-supplies'

const distributor = distributeOfficeSupplies()
...
distributor.backToSchoolStarted(async ({ weeksOut }) => {
  if (weeksOut === 8) {
