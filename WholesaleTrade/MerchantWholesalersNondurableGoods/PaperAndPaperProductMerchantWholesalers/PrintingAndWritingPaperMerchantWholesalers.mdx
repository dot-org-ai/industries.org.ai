...
| millAllocationAnnounced | Supply constraint announced |
| gradeDiscontinued | Paper grade ending production |
| qualityIssueDetected | Paper defect reported |
...
```typescript
import { distributePrintingPaper } from '@headlessly/distribute-printing-paper'

const distributor = distributePrintingPaper()
...
distributor.stockDepleted(async ({ grade, currentLevel, reorderPoint }) => {
