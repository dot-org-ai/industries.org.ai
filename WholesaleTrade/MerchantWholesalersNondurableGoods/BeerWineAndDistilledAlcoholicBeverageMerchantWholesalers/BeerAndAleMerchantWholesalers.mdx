...
| DraftTechnician | Installs, cleans, and maintains draft beer systems at retail accounts |
...
| productExpirationNeared | Beer approaching freshness date |
...
// Search catalog by brand, style, or package
...
distributor.productExpirationNeared(async ({ productId, lotId, daysRemaining }) => {
  if (daysRemaining <= 30) {
...
distributor.inventoryDepleted(async ({ productId, currentStock, reorderPoint }) => {
