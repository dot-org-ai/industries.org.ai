...
| inventoryLow | Tank level fell below threshold |
| inventoryVariance | Gain or loss detected |
| complianceAlert | Environmental deadline approaching |
...
terminal.inventoryLow(async ({ tankId, product, currentLevel, threshold }) => {
...
terminal.inventoryVariance(async ({ tankId, bookVolume, physicalVolume }) => {
