...
| tankDepleted | Tank level fell below threshold |
| serviceRequired | Equipment maintenance needed |
| priceChanged | Rack or customer pricing updated |
| truckMaintenanceBecameDue | Vehicle service due |
| creditRiskDetected | Customer credit risk threshold breached |
...
jobber.tankDepleted(async ({ customerId, product, currentLevel, capacity }) => {
...
jobber.creditRiskDetected(async ({ customerId, invoiceId, daysPastDue, amount }) => {
  if (daysPastDue >= 30) {
