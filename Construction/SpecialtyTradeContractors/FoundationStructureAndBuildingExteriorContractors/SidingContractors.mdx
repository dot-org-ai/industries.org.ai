...
| sealingCompleted | All penetrations caulked |
...
```typescript
import { installSiding } from '@headlessly/install-siding'

const sider = installSiding()

// Check weather conditions for scheduling
const weather = await sider.getWeatherForecast({ location: 'home-123' })

// Review pending estimates
const estimates = await sider.getEstimates({ status: 'pending' })
