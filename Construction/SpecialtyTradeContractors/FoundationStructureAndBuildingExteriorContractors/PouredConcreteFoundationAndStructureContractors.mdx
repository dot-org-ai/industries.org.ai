...
// Review upcoming pour schedule
const pours = await contractor.getPourSchedule({ week: 'current' })

// Check cylinder break test results
const breaks = await contractor.getCylinderBreaks({ projectId: 'gc-project-123' })
