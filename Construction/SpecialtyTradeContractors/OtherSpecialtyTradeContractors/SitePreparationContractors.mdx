...
```typescript
import { gradeSitePreparation } from '@headlessly/grade-site-preparation'

const siteContractor = gradeSitePreparation()

// Review cut/fill earthwork calculations
const earthwork = await siteContractor.getCutFillQuantities({ projectId: 'commercial-plaza-456' })

// Check equipment availability
const equipment = await siteContractor.getEquipmentStatus({ type: 'excavator', status: 'available' })
