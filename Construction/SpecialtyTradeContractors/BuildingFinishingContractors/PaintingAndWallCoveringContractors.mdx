...
| specialtyFinishCompleted | Decorative finish complete |
| touchUpCompleted | Corrections finished |
...
// Review approved color schedule
const colors = await painter.getColorSchedule({ projectId: 'office-repaint-456' })

// Check exterior painting conditions
const weather = await painter.getWeatherForecast({ location: 'office-repaint-456' })
...
painter.touchUpCompleted(async ({ projectId }) => {
