...
| Event | Description |
|-------|-------------|
| takeoffCompleted | Material quantities calculated |
| materialsDelivered | Drywall and insulation on site |
| metalFramingCompleted | Stud walls and ceilings framed |
| insulationInstalled | Insulation placed and inspected |
| drywallHung | Gypsum board attached |
| tapingCompleted | First coat finished |
| finishingCompleted | Final coats applied |
...
| firestoppingCompleted | Penetrations sealed |
...
// Check material requirements for the project
const materials = await contractor.getMaterialNeeds({ projectId: 'office-tenant-123' })

// Review fire-rated assembly requirements
const fireRatings = await contractor.getFireRatings({ projectId: 'office-tenant-123' })
...
contractor.finishingCompleted(async ({ projectId, area }) => {
