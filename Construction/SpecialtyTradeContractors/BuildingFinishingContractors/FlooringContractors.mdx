...
| walkthroughCompleted | Customer accepted work |
...
```typescript
import { finishFlooring } from '@headlessly/finish-flooring'

const floorer = finishFlooring()

// Check material delivery and acclimation status
const deliveries = await floorer.getDeliveryStatus({ projectId: 'office-buildout-123' })
const acclimation = await floorer.getAcclimationStatus({ projectId: 'office-buildout-123', area: 'main-floor' })
