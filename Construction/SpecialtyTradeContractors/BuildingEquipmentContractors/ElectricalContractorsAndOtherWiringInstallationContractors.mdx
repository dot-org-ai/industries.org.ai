...
| Event | Description |
|-------|-------------|
| loadCalculationCompleted | Service requirements determined |
| shopDrawingsApproved | Coordination drawings accepted |
| roughInCompleted | Conduit and boxes installed |
| equipmentSet | Panels and switchgear mounted |
| wirePulled | Conductors installed in conduit |
| terminationsCompleted | All connections made |
...
```typescript
import { installElectricalSystems } from '@headlessly/install-electrical-systems'

const electrician = installElectricalSystems()

// Review panel schedule and circuit assignments
const circuits = await electrician.getCircuitSchedule({ projectId: 'office-building-456' })

// Check electrical material delivery status
const materials = await electrician.getMaterialOrders({ projectId: 'office-building-456' })
...
electrician.roughInCompleted(async ({ projectId, area }) => {
