...
| phaseCompleted | All lots in phase delivered |
...
// Check entitlement status for active projects
const entitlements = await developer.getEntitlementStatus({ projectId: 'APN-789-012' })

// Review available lots ready for sale
const lots = await developer.findAvailableLots({ phase: 1, status: 'improved' })
...
developer.roadwaysCompleted(async ({ projectId, phase }) => {
