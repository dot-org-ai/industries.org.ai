...
// Check pipeline installation progress
const progress = await contractor.getPipelineStatus({ projectId: 'ws-project-123', segment: 'Main-St' })

// Review scheduled pipe deliveries
const deliveries = await contractor.getMaterialDeliveries({ projectId: 'ws-project-123' })
