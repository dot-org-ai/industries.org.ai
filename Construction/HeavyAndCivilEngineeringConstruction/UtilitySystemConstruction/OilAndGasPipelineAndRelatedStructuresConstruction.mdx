...
| Pipeliner | Skilled tradesperson aligning, welding, and coating pipeline joints |
| SideboomOperator | Operates tracked sideboom tractors for pipe lifting and lowering-in |
...
| crossingCompleted | HDD or bore crossing installed |
| stationCompleted | Compressor or meter station operational |
...
```typescript
import { installOilGasPipelines } from '@headlessly/install-oil-gas-pipelines'

const contractor = installOilGasPipelines()

// Check right-of-way acquisition progress
const rowStatus = await contractor.getRightOfWayStatus({ projectId: 'pipeline-456', segment: 'MP-0-50' })

// Review weld inspection log
const welds = await contractor.getWeldLog({ projectId: 'pipeline-456', spread: 'spread-1' })
