...
| trafficControlActive | Lane closures and detours in place |
| earthworkComplete | Grading finished to subgrade |
| drainageInstalled | Storm drainage systems complete |
| bridgeSubstructureComplete | Foundations and piers finished |
| bridgeSuperstructureComplete | Deck and barriers finished |
| baseCourseComplete | Base materials installed |
| pavementComplete | Final surface course placed |
| guardrailInstalled | Safety barriers in place |
| stripingComplete | Markings and signs installed |
...
const payEstimate = await contractor.getPayEstimate({ projectId: 'DOT-2024-0456' })

// Submit bid proposal
const bid = await contractor.submitBid({
  projectId: 'DOT-2024-0456',
...
contractor.baseCourseComplete(async ({ projectId, station }) => {
  await contractor.paveSurface({
    projectId,
    station,
...
contractor.stripingComplete(async ({ projectId }) => {
  await contractor.conductFinalInspection({
    projectId,
    requestedDate: addDays(new Date(), 7),
    inspectorType: 'DOT-resident-engineer'
  })
})
```
