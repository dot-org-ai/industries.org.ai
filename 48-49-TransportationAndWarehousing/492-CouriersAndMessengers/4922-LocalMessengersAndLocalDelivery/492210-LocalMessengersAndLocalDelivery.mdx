...
| enRoutePickup | Heading to sender |
| arrivedPickup | At sender location |
| itemPickedUp | Collected and verified |
| enRouteDropoff | Heading to recipient |
| arrivedDropoff | At delivery location |
...
```typescript
import { deliverLocalMessengers } from '@headlessly/deliver-local-messengers'

const delivery = deliverLocalMessengers()
...
delivery.jobAccepted(async ({ deliveryId, driverId }) => {
  setTimeout(async () => {
    ...
    if (status.phase === 'accepted' && !status.enRoutePickup) {
