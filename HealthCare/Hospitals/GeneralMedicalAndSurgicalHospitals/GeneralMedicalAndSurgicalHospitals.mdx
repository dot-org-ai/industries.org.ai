...
| bedBecameAvailable | Bed is ready for new patient |
| codeBlueCalled | Cardiac arrest response activated |
...
graph LR
    ...
    H -->|buys from| MedicalSupplyVendor
...
// Check bed availability and current census
const beds = await hospital.getAvailableBeds({ unit: 'surgical-floor', type: 'semi-private' })
const census = await hospital.getCensus({ unit: 'surgical-floor' })
