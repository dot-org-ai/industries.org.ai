...
| authorizationExpiring | Insurance approval for continued stay is approaching expiration |
| dischargeApproaching | Resident target discharge date is nearing |
...
facility.authorizationExpiring(async ({ residentId, authorizationId, daysRemaining }) => {
  if (daysRemaining <= 3) {
...
facility.dischargeApproaching(async ({ residentId, targetDate }) => {
