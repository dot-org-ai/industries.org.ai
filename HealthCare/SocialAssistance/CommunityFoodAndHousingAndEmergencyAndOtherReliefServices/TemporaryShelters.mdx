...
// Check bed availability and current waitlist
const beds = await shelter.getAvailableBeds({ type: 'single', accessible: true })
const waitlist = await shelter.getWaitList({ priority: 'veteran' })
