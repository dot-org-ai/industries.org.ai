...
// Check supply inventory and active disaster zones
const inventory = await reliefOrg.getSupplyInventory({ warehouse: 'gulf-coast', category: 'emergencyKits' })
const activeDisasters = await reliefOrg.getActiveDisasters({ region: 'southeast', severity: 'major' })
