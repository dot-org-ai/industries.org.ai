...
| AgriculturalRegulator | Provides commodity foods and program oversight |
...
graph LR
    ...
    O -->|reports to| AgriculturalRegulator
...
// Check food inventory levels and delivery routes
const inventory = await foodBank.getInventory({ category: 'protein', belowMinimum: true })
const deliveryRoutes = await foodBank.getDeliveryRoutes({ date: '2024-04-01' })
...
foodBank.inventoryDepleted(async ({ itemType, currentQuantity }) => {
