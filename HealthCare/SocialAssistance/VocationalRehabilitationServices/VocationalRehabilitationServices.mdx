...
| retentionMilestoneReached | Client has reached an employment retention milestone |
...
// Find available training programs for client placement
const programs = await vocRehab.findTrainingPrograms({ skillArea: 'officeAdmin', format: 'inPerson' })

// Check current job leads in target field
const leads = await vocRehab.getJobLeads({ industry: 'administrative', accommodations: true })
...
vocRehab.retentionMilestoneReached(async ({ clientId, placementId, daysEmployed }) => {
  if (daysEmployed === 90) {
