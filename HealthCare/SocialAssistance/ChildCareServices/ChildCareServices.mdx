...
| EarlyInterventionist | Identifies and supports developmental delays in infants and toddlers |
...
| pickupTimeNeared | Scheduled end-of-day pickup time is nearing for a child |
| enrollmentAnniversaryReached | Child has reached a periodic enrollment milestone |
...
// Check classroom availability and waitlist
const waitlist = await center.getWaitList({ program: 'fullDayPreschool' })
const classrooms = await center.getClassrooms({ ageGroup: 'preschool', hasOpenings: true })
...
center.pickupTimeNeared(async ({ childId, scheduledPickup }) => {
...
center.enrollmentAnniversaryReached(async ({ childId, enrollmentId, monthsEnrolled }) => {
