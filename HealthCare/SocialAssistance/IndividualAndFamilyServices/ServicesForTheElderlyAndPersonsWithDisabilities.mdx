...
| wellbeingConcernRaised | A concern about client health or safety has been identified |
| serviceScheduled | A service visit or program session has been scheduled |
...
const agency = assistSeniors()

// Review current caseload by service type
const clients = await agency.findClients({ status: 'active', service: 'homeCare' })
...
agency.wellbeingConcernRaised(async ({ clientId, concernType, severity }) => {
  await agency.supportCaregiver({
    clientId,
    alertType: 'wellbeingConcernRaised',
    concernType,
    severity
  })
