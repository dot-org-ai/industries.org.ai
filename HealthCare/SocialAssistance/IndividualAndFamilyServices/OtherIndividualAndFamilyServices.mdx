...
| crisisReported | Client emergency situation has been reported |
| milestoneReached | Client has reached a service plan progress milestone |
...
```typescript
import { counselFamilies } from '@headlessly/counsel-families'

const agency = counselFamilies()

// Review active crisis interventions and caseload
const crises = await agency.findCrisisInterventions({ status: 'active', severity: 'high' })
const cases = await agency.findClients({ status: 'active', counselor: 'lc-001' })
...
agency.milestoneReached(async ({ caseId, milestone, sessionCount }) => {
