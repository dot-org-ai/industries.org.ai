...
| authorizationExpirationNeared | Approval is nearing expiration |
...
graph LR
    ...
    R -->|buys from| HearingAidManufacturer
```
...
// Review pending evaluations and expiring authorizations
const pendingEvals = await practice.getPendingEvaluations({ therapyType: 'physical-therapy' })
const expiringAuths = await practice.getExpiringAuthorizations({ expiresWithin: '14-days' })
...
practice.authorizationExpirationNeared(async ({ patientId, visitsRemaining }) => {
