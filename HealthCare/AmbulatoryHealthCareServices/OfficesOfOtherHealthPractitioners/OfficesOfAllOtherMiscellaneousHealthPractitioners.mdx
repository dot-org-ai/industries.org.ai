...
```typescript
import { treatAlternativeCare } from '@headlessly/treat-alternative-care'

const practice = treatAlternativeCare()

// Review active treatment plans and prenatal patients
const activePlans = await practice.getActiveTreatmentPlans({ practiceType: 'acupuncture' })
const prenatalPatients = await practice.getPrenatalPatients({ trimester: 'third' })
