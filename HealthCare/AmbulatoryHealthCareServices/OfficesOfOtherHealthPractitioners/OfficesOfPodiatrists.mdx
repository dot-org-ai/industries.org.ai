...
// Check patients with active wounds and diabetic patients needing monitoring
const activeWounds = await practice.getActiveWounds({ status: 'open', woundType: 'ulcer' })
const diabeticPatients = await practice.getDiabeticPatients({ dueForExam: true })
