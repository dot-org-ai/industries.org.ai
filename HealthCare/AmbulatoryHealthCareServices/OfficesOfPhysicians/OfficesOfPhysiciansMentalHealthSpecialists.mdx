...
| medicationRefillBecameDue | Prescription renewal is needed |
...
const highRisk = await practice.getHighRiskPatients({ activeSafetyPlan: true })

// Review pending evaluations
const pendingEvals = await practice.getPendingEvaluations({ status: 'scheduled' })
