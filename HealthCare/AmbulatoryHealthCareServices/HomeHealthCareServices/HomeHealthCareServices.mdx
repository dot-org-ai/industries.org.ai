...
| HealthRegulatorSurveyor | Inspects agency and enforces conditions of participation |
...
| WoundCareNurse | Provides specialized wound assessment and treatment in the home setting |
...
| episodeEnded | Current 60-day care episode is approaching its end date |
| goalsMet | Patient has achieved care plan goals |
...
graph LR
    ...
    H -->|inspected by| HealthRegulatorSurveyor
```
...
// Check pending admissions and recertifications due
const pendingAdmissions = await agency.getPendingAdmissions({ status: 'awaiting-start-of-care' })
const recerts = await agency.getDueRecertifications({ dueWithin: '14-days' })
...
agency.episodeEnded(async ({ patientId, episodeEndDate, daysRemaining }) => {
  if (daysRemaining <= 5) {
