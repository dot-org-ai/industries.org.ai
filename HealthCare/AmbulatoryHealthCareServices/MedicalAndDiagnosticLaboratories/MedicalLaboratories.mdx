...
| HealthRegulator | Oversees CLIA certification and compliance |
...
graph LR
    ...
    L -->|certified by| HealthRegulator
    L -->|accredited by| CAP
    L -->|tests at| ReferenceLabratory
    L -->|licensed by| StateHealthDepartment
```
...
// Check for unreported critical values and pending tests
const criticals = await lab.getCriticalValues({ status: 'unreported' })
const pending = await lab.getPendingTests({ priority: 'stat' })
