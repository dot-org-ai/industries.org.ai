...
| drugScreenTestedPositive | Substance detected in screening |
...
| authorizationExpirationNeared | Insurer approval for services is approaching expiration |
...
// Review court-mandated caseload and pending authorizations
const courtMandated = await center.getCourtMandatedPatients({ complianceStatus: 'active' })
const pendingAuth = await center.getPendingAuthorizations({ expiresWithin: '7-days' })
...
center.drugScreenTestedPositive(async ({ patientId, substance }) => {
...
center.authorizationExpirationNeared(async ({ patientId, daysRemaining }) => {
  if (daysRemaining <= 5) {
