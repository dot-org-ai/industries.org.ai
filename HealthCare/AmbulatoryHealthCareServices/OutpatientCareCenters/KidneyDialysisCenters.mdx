...
| HealthRegulatorSurveyor | Inspects facility and enforces regulations |
...
| DialysisTechnician | Operates dialysis equipment and monitors fluid removal during treatments |
...
graph LR
    ...
    D -->|inspected by| HealthRegulatorSurveyor
```
...
// Check treatment schedule and access issues for the shift
const schedule = await center.getSchedule({ day: 'monday', shift: 'AM' })
const accessIssues = await center.getAccessIssues({ status: 'unresolved' })
