...
| FlightParamedic | Provides critical care interventions during helicopter air transport |
...
| unitEnRouted | Ambulance is traveling to incident |
| unitArrivedOnScene | Ambulance has arrived at location |
| patientContacted | Crew has made contact with patient |
...
| arrivedAtHospital | Ambulance has reached facility |
| careTransferred | Patient has been handed off |
| unitBecameAvailable | Ambulance is ready for next call |
...
// Check unit availability and active calls
const units = await service.getUnitStatus({ zone: 'central', status: 'available' })
const activeCalls = await service.getActiveCalls({ priority: 'emergency' })
