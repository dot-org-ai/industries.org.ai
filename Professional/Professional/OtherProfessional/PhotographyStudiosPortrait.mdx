...
```typescript
import { photographPortraits } from '@headlessly/photograph-portraits'

const studio = photographPortraits()

// Find upcoming sessions for the week
const sessions = await studio.findSessions({ week: 'current', status: 'confirmed' })

// Check client galleries for past shoots
const galleries = await studio.getGalleries({ clientId: 'client-123', status: 'ready-for-review' })
