...
// Find available linguists for the language pair
const linguists = await language.searchLinguists({ sourceLang: 'en', targetLang: 'ja', specialty: 'legal' })

// Check active translation projects
const projects = await language.findProjects({ status: 'in-progress', language: 'japanese' })
