...
| calibrationBecameDue | Instrument requires accuracy verification |
...
```typescript
import { testSamples } from '@headlessly/test-samples'

const lab = testSamples()

// Check calibration status of instruments
const calibration = await lab.getCalibrationStatus({ dueWithinDays: 30 })

// Find pending samples for the day
const pending = await lab.findSamples({ status: 'received', testDate: '2024-06-15' })
...
lab.calibrationBecameDue(async ({ instrumentId, dueDate, calibrationType }) => {
