...
```typescript
import { surveyTerrain } from '@headlessly/survey-terrain'

const geo = surveyTerrain()

// Find active surveys in the region
const surveys = await geo.findSurveys({ region: 'gulf-coast', status: 'planned' })

// Check survey line progress
const lines = await geo.getSurveyLines({ surveyId: 'survey-2024-001', status: 'incomplete' })
