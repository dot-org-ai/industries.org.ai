...
| substantialCompletionReached | Building ready for occupancy |
...
const references = await arch.findProjects({ projectType: 'commercial', status: 'completed' })

// Check open RFIs requiring response
const rfis = await arch.findRFIs({ status: 'pending', projectId: 'project-456' })
