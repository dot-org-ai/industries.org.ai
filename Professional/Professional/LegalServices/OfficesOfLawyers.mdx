...
| deadlineNeared | Statutory or court deadline within warning threshold |
...
```typescript
import { litigateLegalCases } from '@headlessly/litigate-legal-cases'

const firm = litigateLegalCases()

// Check upcoming deadlines across active matters
const deadlines = await firm.getDeadlines({ daysAhead: 14, practiceArea: 'corporate' })

// Review outstanding balances
const balances = await firm.getOutstandingBalances({ ageOver: 60, minAmount: 5000 })
...
firm.deadlineNeared(async ({ matterId, deadline, daysRemaining }) => {
  if (daysRemaining <= 3) {
