...
| deadlineNeared | Filing or service deadline within warning threshold |
...
```typescript
import { adviseLegalCases } from '@headlessly/advise-legal-cases'

const legal = adviseLegalCases()

// Find open process service assignments
const pending = await legal.findServiceJobs({ status: 'pending', region: 'denver-metro' })

// Check upcoming filing deadlines
const deadlines = await legal.getDeadlines({ daysAhead: 7, type: 'service' })
...
legal.deadlineNeared(async ({ serviceId, deadline, daysRemaining }) => {
  if (daysRemaining <= 5) {
