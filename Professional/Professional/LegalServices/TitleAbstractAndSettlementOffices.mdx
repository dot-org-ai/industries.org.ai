...
| documentsPrepared | Closing package prepared for signing |
...
// List files by status, property address, or party name
const transactions = await title.findTransactions({ status: 'in-progress', county: 'Travis' })

// Retrieve ownership history for property
const chainOfTitle = await title.getChainOfTitle({ parcelNumber: '12345-678-90' })
