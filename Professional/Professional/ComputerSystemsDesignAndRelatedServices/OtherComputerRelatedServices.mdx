...
// Check backup status for all client systems
const backups = await itServices.searchBackups({ status: 'failed', dateRange: 'last-24-hours' })

// Review pending migration projects
const migrations = await itServices.getMigrationStatus({ status: 'in-progress' })
