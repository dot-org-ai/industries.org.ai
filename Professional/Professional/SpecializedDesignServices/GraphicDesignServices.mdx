...
| productionPrepared | Files prepared for output |
...
| deadlineNeared | Project milestone reached warning threshold |
...
const projects = await design.findProjects({ status: 'in-progress', clientId: 'client-acme' })

// Check design versions awaiting approval
const versions = await design.getDesignVersions({ projectId: 'project-brand', status: 'pending-review' })
