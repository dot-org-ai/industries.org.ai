...
| TaxAuthority | Federal agency receiving employment tax deposits |
...
| GarnishmentAdministrator | Manages court-ordered wage withholding and remittance |
...
| deadlineNeared | Tax deposit or filing deadline nearing |
...
// Find payrolls with pending issues
const pendingPayrolls = await payroll.findPayrolls({ status: 'pending-review', clientId: 'client-123' })
...
payroll.deadlineNeared(async ({ clientId, deadline, type, amount }) => {
