...
| TaxAuthority | Internal Revenue Service processing federal returns |
...
| fileElectronically | Transmit return to tax authority and state agencies |
...
| returnAccepted | Tax authority acknowledged receipt of return |
| returnRejected | Filing error requiring correction |
| refundIssued | Taxpayer received refund |
| deadlineNeared | Filing deadline within warning threshold |
...
// Find today's scheduled appointments
const todayAppts = await tax.findAppointments({ date: '2024-03-25', status: 'confirmed' })

// Check prior year returns for returning client
const priorReturns = await tax.getClientReturns({ clientId: 'client-smith', years: [2022, 2023] })
...
tax.deadlineNeared(async ({ clientId, deadline, daysRemaining }) => {
  if (daysRemaining <= 7) {
