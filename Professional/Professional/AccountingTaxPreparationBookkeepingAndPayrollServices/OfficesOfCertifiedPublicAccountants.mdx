...
| SEC | Securities and Exchange Commission requiring filings |
| IRS | Internal Revenue Service administering tax law |
...
| deadlineApproaching | Filing or delivery date within warning threshold |
...
cpa.deadlineApproaching(async ({ engagementId, deadline, daysRemaining }) => {
  if (daysRemaining <= 7) {
