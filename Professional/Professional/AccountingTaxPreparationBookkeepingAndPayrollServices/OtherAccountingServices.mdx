...
| receivableBecameOverdue | Customer payment past due date |
| cashFlowThresholdBreached | Cash flow fell below acceptable threshold |
| deadlineNeared | Reporting or payment deadline nearing |
| reconciliationVarianceDetected | Difference found between book and bank |
...
books.receivableBecameOverdue(async ({ clientId, customerId, invoiceId, amount, daysOverdue }) => {
  if (daysOverdue >= 30) {
...
books.cashFlowThresholdBreached(async ({ clientId, currentBalance, projectedShortfall }) => {
