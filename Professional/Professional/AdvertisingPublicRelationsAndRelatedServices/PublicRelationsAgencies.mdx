...
```typescript
import { publicizeReputation } from '@headlessly/publicize-reputation'

const pr = publicizeReputation()

// Review recent media coverage for the client
const coverage = await pr.getCoverage({ clientId: 'client-123', period: 'last-30-days' })

// Find active PR campaigns
const campaigns = await pr.findCampaigns({ status: 'active', clientId: 'client-123' })
