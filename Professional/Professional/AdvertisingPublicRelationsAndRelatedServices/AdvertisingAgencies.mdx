...
// Find active campaigns for the client
const campaigns = await agency.findCampaigns({ clientId: 'client-456', status: 'active' })

// Check available creative assets
const assets = await agency.getAssets({ campaignId: 'campaign-789', type: 'video' })
