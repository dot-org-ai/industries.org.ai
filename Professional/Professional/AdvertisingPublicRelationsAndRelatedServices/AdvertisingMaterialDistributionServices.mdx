...
// Check available distribution routes
const routes = await distribution.getRoutes({ market: 'chicago-metro', type: 'door-to-door' })

// Find active distribution campaigns
const campaigns = await distribution.findCampaigns({ status: 'active', clientId: 'client-123' })
