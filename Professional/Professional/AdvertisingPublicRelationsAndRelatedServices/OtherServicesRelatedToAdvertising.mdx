...
// Search promotional product catalog
const products = await promo.searchProducts({ category: 'apparel', minQuantity: 500, maxUnitPrice: 15 })

// Find active promotional orders
const orders = await promo.findOrders({ clientId: 'client-tech', status: 'in-production' })
