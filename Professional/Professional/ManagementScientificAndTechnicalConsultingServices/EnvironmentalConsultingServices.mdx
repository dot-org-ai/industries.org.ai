...
| remediationCompleted | Cleanup activities finished |
...
// Find active environmental projects
const projects = await environmental.findProjects({ status: 'active', type: 'phase-II' })

// Check past assessments for the site
const prior = await environmental.getAssessments({ siteId: 'site-456', type: 'phase-I' })
...
environmental.remediationCompleted(async ({ projectId, finalResults }) => {
