...
// Pull market research data for the sector
const marketData = await marketing.getMarketData({ sector: 'consumer-electronics', region: 'north-america' })

// Review active consulting engagements
const engagements = await marketing.findEngagements({ status: 'active', serviceType: 'brand-strategy' })
