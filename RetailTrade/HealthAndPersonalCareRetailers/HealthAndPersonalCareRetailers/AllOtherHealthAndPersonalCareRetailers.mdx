...
// Search products by type or category
const findEquipmentResult = await store.findEquipment({ status: 'active' })

// Retrieve patient history and equipment
const getPatientRecordResult = await store.getPatientRecord({ status: 'active' })
