...
// Search by patient, Rx number, or prescriber
const findPrescriptionResult = await pharmacy.findPrescription({ status: 'active' })

// Verify formulary status and copay
const checkDrugCoverageResult = await pharmacy.checkDrugCoverage({ status: 'active' })
