...
// Search by brand, category, or ingredient
const findProductsResult = await store.findProducts({ category: 'featured', inStock: true })

// Retrieve beauty preferences and history
const getCustomerProfileResult = await store.getCustomerProfile({ status: 'active' })
