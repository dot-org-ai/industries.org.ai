...
| tankLevelDropped | Tank level fell below threshold |
...
const getFuelPricesResult = await station.getFuelPrices({ status: 'active' })

// Fuel inventory by tank
const getTankLevelsResult = await station.getTankLevels({ status: 'active' })
...
station.tankLevelDropped(async ({ tankId, grade, currentGallons }) => {
