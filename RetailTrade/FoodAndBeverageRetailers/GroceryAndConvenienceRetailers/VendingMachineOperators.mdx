...
```typescript
import { deployVendingMachines } from '@headlessly/deploy-vending-machines'

const operator = deployVendingMachines()

// Search by location or status
const findMachinesResult = await operator.findMachines({ status: 'active' })

// List service routes
const getRoutesResult = await operator.getRoutes({ status: 'active' })
