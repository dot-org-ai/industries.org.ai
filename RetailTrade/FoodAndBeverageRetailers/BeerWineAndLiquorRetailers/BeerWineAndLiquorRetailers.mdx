...
// Search by region, variety, price
const findWinesResult = await store.findWines({ status: 'active' })

// Search by style, brewery, origin
const findBeersResult = await store.findBeers({ status: 'active' })
