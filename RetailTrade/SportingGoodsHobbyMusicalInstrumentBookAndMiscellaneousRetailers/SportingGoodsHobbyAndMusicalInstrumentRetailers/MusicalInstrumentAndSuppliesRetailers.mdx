...
// Search by type, brand, or price
const findInstrumentsResult = await store.findInstruments({ status: 'active' })

// Search by title, composer, or instrument
const findSheetMusicResult = await store.findSheetMusic({ status: 'active' })
