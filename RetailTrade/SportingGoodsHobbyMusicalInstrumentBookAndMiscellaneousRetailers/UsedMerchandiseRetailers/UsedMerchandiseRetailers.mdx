...
| Event | Description |
|-------|-------------|
| consignmentReceived | Item accepted for consignment |
...
// Search by category, brand, or condition
const findItemsResult = await store.findItems({ status: 'active' })

// Consignment items by consignor or status
const getConsignmentsResult = await store.getConsignments({ status: 'active' })
