...
| featuresDemonstrated | Product demonstration completed |
...
// Search by size, brand, or features
const findLuggageResult = await store.findLuggage({ status: 'active' })

// Search bags by style or brand
const findHandbagsResult = await store.findHandbags({ status: 'active' })
