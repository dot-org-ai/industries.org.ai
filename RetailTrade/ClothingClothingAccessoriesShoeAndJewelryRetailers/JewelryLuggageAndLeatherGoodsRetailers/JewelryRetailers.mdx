...
// Search by type, metal, stone, or price
const findJewelryResult = await store.findJewelry({ status: 'active' })

// Search timepieces by brand or style
const findWatchesResult = await store.findWatches({ status: 'active' })
