...
| SubstituteTeacher | Fills in for absent teachers on short notice across grade levels and subjects |
| SchoolNurse | Provides on-site health screenings, first aid, and medication administration to students |
...
// Check enrollment numbers by grade level
const enrollments = await school.getEnrollments({ year: '2024-2025' })

// Review assessment results for a class
const scores = await school.getAssessmentResults({ grade: 5, test: 'state-math' })
