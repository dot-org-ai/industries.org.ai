...
| JourneyworkerMentor | Licensed tradesperson who trains and supervises apprentices during on-the-job learning |
...
// Find registered program sponsors by trade
const sponsors = await program.getSponsors({ trade: 'electrical' })

// List available apprenticeship programs
const programs = await program.getPrograms({ trade: 'electrical', status: 'accepting' })
