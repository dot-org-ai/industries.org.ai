...
| PostdoctoralResearcher | Conducts funded research under faculty mentorship after completing a doctoral degree |
...
// Query active research grants by department
const grants = await university.getGrants({ department: 'biology', status: 'active' })

// List faculty by research area
const faculty = await university.getFaculty({ department: 'biology', area: 'genomics' })
