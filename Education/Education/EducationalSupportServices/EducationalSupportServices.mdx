...
```typescript
import { consultEducationalPrograms } from '@headlessly/consult-educational-programs'

const provider = consultEducationalPrograms()

// List active service contracts
const contracts = await provider.getContracts({ status: 'active' })

// Query assessment results by type
const assessments = await provider.getAssessments({ type: 'standardized', year: 2024 })
