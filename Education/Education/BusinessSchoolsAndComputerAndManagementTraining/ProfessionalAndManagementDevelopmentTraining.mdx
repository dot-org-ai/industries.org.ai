...
| ExecutiveCoach | Provides one-on-one leadership development and behavioral coaching to senior professionals |
...
// Find scheduled workshops by topic
const workshops = await provider.getWorkshops({ topic: 'leadership', month: 6 })

// Review participant feedback by program
const feedback = await provider.getFeedback({ program: 'executive-leadership-2024' })
