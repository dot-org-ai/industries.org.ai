...
graph LR
    ...
    T -->|partners with| IndustryPartner
```
...
// List courses by technology stack
const courses = await center.getCourses({ technology: 'cloud-computing', level: 'intermediate' })

// Check lab availability for hands-on training
const labs = await center.getLabSchedule({ date: new Date() })
