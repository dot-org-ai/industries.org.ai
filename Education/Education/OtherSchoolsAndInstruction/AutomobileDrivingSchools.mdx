...
| LicensingAuthority | Department of Motor Vehicles issuing permits and licenses |
...
| Permit | Learner authorization from licensing authority |
| License | Full driving authorization from licensing authority |
...
| prepareRoadTest | Ready student for practical driving exam |
...
| scheduleRoadTest | Book road test appointment for student |
...
| roadTestPrepared | Road test readiness confirmed |
...
| roadTestScheduled | Road test appointment booked |
...
graph LR
    ...
    S -->|prepares for| LicensingAuthority
    ...
    if (practiceHours.total >= student.state.requiredHours) {
      ...
      await sendNotification({
        ...
        data: {
          ...
          nextSteps: 'Schedule road test appointment'
        }
      })
    }
  }
