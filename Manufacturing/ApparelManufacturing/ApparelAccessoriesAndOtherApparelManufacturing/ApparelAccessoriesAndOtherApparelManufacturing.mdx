...
```typescript
import { manufactureApparelAccessories } from '@headlessly/manufacture-apparel-accessories'

const manufacturer = manufactureApparelAccessories()

// Produce leather belts
const materials = await manufacturer.sourceMaterials({
  leather: { type: 'full-grain-cowhide', color: 'brown', quantity: 200, unit: 'hides' },
  hardware: { type: 'brass-buckle', style: 'single-prong', quantity: 1000 }
})
...
await manufacturer.sew({
  batchId: materials.batchId,
  operations: ['keeper-attach', 'tip-stitch'],
  thread: 'heavy-duty-waxed'
})

await manufacturer.attachHardware({
  batchId: materials.batchId,
  buckle: 'brass-single-prong',
  method: 'chicago-screw'
})
```
...
```typescript
// Auto-finish after hardware attachment
manufacturer.hardwareAttached(async ({ batchId, productType }) => {
  await manufacturer.finish({
    batchId,
    operations: productType === 'belt'
      ? ['edge-burnish', 'edge-paint', 'condition']
      : ['trim-threads', 'steam']
  })
})

// Auto-inspect after finishing
manufacturer.finishingCompleted(async ({ batchId }) => {
  await manufacturer.inspect({
    batchId,
    checks: ['hardware-security', 'stitch-quality', 'edge-finish', 'sizing']
  })
})
...
manufacturer.inspectionPassed(async ({ batchId, customer }) => {
  await manufacturer.label({
    batchId,
    labels: ['brand', 'size', 'care', 'upc']
  })

  await manufacturer.package({
    batchId,
    method: customer.includes('retail') ? 'box-with-tissue' : 'poly-bag',
    cartonQuantity: 24
  })
})

// Handle sample requests
manufacturer.designApproved(async ({ styleNumber, customer }) => {
  if (customer.requiresSample) {
    await manufacturer.sourceMaterials({
      styleNumber,
      sampleQuantity: true
    })
  }
