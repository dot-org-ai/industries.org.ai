...
| fabricKnitted | Greige fabric produced |
...
```typescript
import { knitKnitFabric } from '@headlessly/knit-knit-fabric'

const mill = knitKnitFabric()
...
mill.fabricKnitted(async ({ lotId, fabricType }) => {
  await mill.inspect({
    lotId,
    method: 'light-box',
    samplingRate: 10
