...
// List incoming fabric inventory
const greigeLots = await finisher.findGreigeLots({ fabric: 'cotton-twill', status: 'received' })

// Check finished fabric stock
const inventory = await finisher.getInventory({ fabricType: 'cotton-twill', finish: 'dyed' })
