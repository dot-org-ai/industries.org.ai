...
| Cooper | Maintains, repairs, and chars oak barrels used for aging spirits |
...
| fermentationCompleted | Wash ready for distillation |
| distillationCompleted | Spirit collected |
| cutsCompleted | Hearts separated from heads and tails |
...
```typescript
import { distillSpirits } from '@headlessly/distill-spirits'

const distillery = distillSpirits()

// Check available stock
const stock = await distillery.findBatches({ status: 'available' })

// Check finished goods inventory
const inventory = await distillery.getBarrelInventory({ lowStockOnly: true })
...
distillery.fermentationCompleted(async ({ batchId, abv }) => {
...
distillery.cutsCompleted(async ({ batchId, heartsVolume, proof }) => {
