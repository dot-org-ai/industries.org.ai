...
| fermentationComplete | Sugars converted to alcohol |
...
```typescript
import { brewWineries } from '@headlessly/brew-wineries'

const winery = brewWineries()
...
winery.fermentationComplete(async ({ lotId, variety, residualSugar }) => {
  if (residualSugar < 2) {
    await winery.press({
      lotId,
