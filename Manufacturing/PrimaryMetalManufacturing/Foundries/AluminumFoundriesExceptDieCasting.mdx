...
```typescript
import { castAluminumCastings } from '@headlessly/cast-aluminum-castings'

const foundry = castAluminumCastings()

// Check available stock
const stock = await foundry.findAlloyStock({ status: 'available' })

// Review pending orders
const orders = await foundry.getOrders({ status: 'pending' })
