...
```typescript
import { refineAluminaAluminum } from '@headlessly/refine-alumina-aluminum'

const smelter = refineAluminaAluminum()

// List available bauxite by grade and source
const bauxiteStock = await smelter.findBauxiteStock({ grade: 'gibbsite', minTons: 5000 })

// Check alumina stock by grade
const aluminaInventory = await smelter.getAluminaInventory({ grade: 'smelter' })
