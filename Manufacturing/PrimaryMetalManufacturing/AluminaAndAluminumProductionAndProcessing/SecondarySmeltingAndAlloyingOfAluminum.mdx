...
```typescript
import { smeltRecycledAluminum } from '@headlessly/smelt-recycled-aluminum'

const recycler = smeltRecycledAluminum()

// List available scrap by type and grade
const scrapInventory = await recycler.findScrapInventory({ grade: 'twitch', minLbs: 20000 })

// Retrieve pending orders by alloy
const orderQueue = await recycler.getOrderQueue({ alloy: 'A356', status: 'pending' })
