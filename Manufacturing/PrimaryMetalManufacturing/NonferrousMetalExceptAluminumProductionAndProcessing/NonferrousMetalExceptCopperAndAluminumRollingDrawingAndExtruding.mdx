...
groupCode: 3314
action: Refine
object: Nonferrous Metal Shapes
---
...
```typescript
import { refineNonferrousMetalShapes } from '@headlessly/refine-nonferrous-metal-shapes'

const mill = refineNonferrousMetalShapes()

// List available ingots by alloy and size
const ingotStock = await mill.findIngotStock({ metal: 'titanium', alloy: 'Ti-6Al-4V' })

// Retrieve open orders by specification
const orders = await mill.getOrders({ product: 'sheet', status: 'open' })
