...
```typescript
import { rollCopperProducts } from '@headlessly/roll-copper-products'

const mill = rollCopperProducts()

// List available cathodes by grade
const cathodeStock = await mill.findCathodeStock({ grade: 'LME-A', minLbs: 40000 })

// Check alloy stock by composition
const alloyInventory = await mill.getAlloyInventory({ alloy: 'C110' })
