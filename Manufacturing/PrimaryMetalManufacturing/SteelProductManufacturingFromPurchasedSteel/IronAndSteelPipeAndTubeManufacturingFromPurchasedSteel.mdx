...
// List available coils by size and grade
const coils = await pipemill.findCoils({ grade: 'A513', gauge: 14 })

// Retrieve open orders by customer and due date
const orders = await pipemill.getOrders({ product: 'roundTube', status: 'open' })
