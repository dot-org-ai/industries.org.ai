...
// List available wire rod by grade and size
const rodStock = await drawer.findRodStock({ grade: '1080', diameter: 5.5 })

// Retrieve open orders by gauge and finish
const orders = await drawer.getOrders({ product: 'wire', status: 'open' })
