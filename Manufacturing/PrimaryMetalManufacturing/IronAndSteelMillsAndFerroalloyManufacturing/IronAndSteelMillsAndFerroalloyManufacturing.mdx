...
| LadleMetallurgist | Manages ladle refining and alloy additions between furnace and caster |
| CasterOperator | Operates continuous casting machine that solidifies molten steel into slabs |
...
// List heats by chemistry, grade, or production date
const heats = await mill.findHeats({ grade: 'A36', status: 'in-process' })

// Retrieve current stock by product type and grade
const inventory = await mill.getInventory({ product: 'hotRolledCoil', grade: 'A36' })
