...
| FoodSafetyInspector | Conducts continuous inspection |
...
| QualityManager | Ensures food safety and regulatory compliance |
...
| birdsInspected | Post-mortem inspection passed |
...
```typescript
import { processPoultry } from '@headlessly/process-poultry'

const plant = processPoultry()

// Check today's processing schedule
...
// Check current product inventory for demand planning
const inventory = await plant.getInventory({ cut: 'boneless-breast', location: 'cold-storage' })
...
await plant.grade({ flockId: flock.id, standard: 'grade-A' })
```
