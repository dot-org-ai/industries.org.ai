...
| agingCompleted | Target age reached |
...
```typescript
import { produceCheese } from '@headlessly/produce-cheese'

const plant = produceCheese()

// Check milk availability and aging cave inventory
const milkLots = await plant.findMilkLots({ butterfatMin: 3.4, status: 'available' })
const agingInventory = await plant.getAgingInventory({ variety: 'cheddar' })
...
plant.agingCompleted(async ({ wheelId, variety, ageMonths }) => {
