...
| FoodSafetyRegulator | Regulates food safety and enrichment requirements |
...
| Enrichment | Vitamins and minerals added per regulatory requirements |
...
```typescript
import { millFlour } from '@headlessly/mill-flour'

const mill = millFlour()

// Check wheat inventory by class and protein
const wheatStock = await mill.findWheatLots({ class: 'hardRedWinter', proteinMin: 12.0 })

// Review pending customer orders
const orders = await mill.findOrders({ status: 'pending', shipBy: 'this-week' })
