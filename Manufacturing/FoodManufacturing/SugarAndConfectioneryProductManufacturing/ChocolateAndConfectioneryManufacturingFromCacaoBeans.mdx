...
| FoodSafetyRegulator | Regulates food safety and chocolate standards |
...
// Check cacao bean inventory by origin
const cacaoStock = await plant.findCacaoLots({ origin: 'ecuador', fermentationMin: 5 })

// Review roast profiles for this origin
const profiles = await plant.getRoastProfiles({ origin: 'ecuador-arriba' })
