...
| monitorPerformance | Track throughput and uptime |
...
| performanceAlert | Throughput or uptime issue |
| monitorPerformance | System performance metrics collected and analyzed |
...
factory.performanceAlert(async ({ systemId, alertType, location }) => {
  if (alertType === 'jam') {
...
factory.monitorPerformance(async ({ systemId, uptimePercent }) => {
