...
| SafetyInspector | Oversees crane safety compliance |
...
| utilizationDataRetrieved | Crane usage statistics retrieved |
...
| utilizationDataRetrieved | Retrieve crane usage statistics |
| findReplacementParts | Query hoist and component inventory |
| getComplianceRecords | Retrieve safety compliance documentation |
...
const status = await factory.getCraneStatus()

// Search catalog
const catalog = await factory.findByFacility()
...
factory.utilizationDataRetrieved(async ({ craneId, hoistCycles }) => {
