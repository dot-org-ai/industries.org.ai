...
// Check overdue maintenance and today's appointments
const overdue = await service.findOverdueServices({ serviceType: 'poolMaintenance', region: 'south' })
const todayJobs = await service.findAppointments({ date: '2024-04-01', status: 'confirmed' })
