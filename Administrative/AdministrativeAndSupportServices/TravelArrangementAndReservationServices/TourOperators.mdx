...
```typescript
import { packageTourExperiences } from '@headlessly/package-tour-experiences'

const tourOp = packageTourExperiences()

// Check existing tours and supplier contracts
const existingTours = await tourOp.findTours({ destination: 'Italy', season: 'fall' })
const contracts = await tourOp.getContracts({ region: 'Tuscany', status: 'active' })
