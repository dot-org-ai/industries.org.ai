...
// Review client roster and pending workers comp claims
const clients = await peo.findClients({ industry: 'technology', status: 'active' })
const claims = await peo.findWorkersCompClaims({ status: 'open' })
