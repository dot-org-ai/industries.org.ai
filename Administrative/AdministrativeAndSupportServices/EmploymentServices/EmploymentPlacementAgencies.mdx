...
```typescript
import { placeJobCandidates } from '@headlessly/place-job-candidates'

const agency = placeJobCandidates()

// Check existing candidates and current pipeline
const matches = await agency.findCandidates({ skills: ['JavaScript', 'React'], location: 'San Francisco' })
const pipeline = await agency.trackPipeline({ employerId: 'tech-corp-123' })
