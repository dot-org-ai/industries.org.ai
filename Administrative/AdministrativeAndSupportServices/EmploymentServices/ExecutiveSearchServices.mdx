...
// Review active engagements and compensation benchmarks
const active = await search.findEngagements({ status: 'active', level: 'C-suite' })
const compData = await search.getCompensationData({ role: 'CFO', market: 'technology', revenue: '1B+' })
