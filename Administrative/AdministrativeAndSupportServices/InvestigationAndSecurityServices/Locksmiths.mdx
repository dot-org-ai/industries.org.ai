...
```typescript
import { serviceLocks } from '@headlessly/service-locks'

const locksmith = serviceLocks()

// Check emergency queue and technician availability
const emergencies = await locksmith.getServiceCalls({ status: 'pending', type: 'emergency' })
const techs = await locksmith.findTechnicians({ availability: 'on-call', specialty: 'residential' })
