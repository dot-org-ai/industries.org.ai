...
```typescript
import { guardPremises } from '@headlessly/guard-premises'

const security = guardPremises()

// Check today's guard schedule and recent incidents
const schedule = await security.getSchedule({ date: '2024-04-01', siteId: 'corporate-tower' })
const incidents = await security.getIncidents({ siteId: 'corporate-tower', daysBack: 7 })
