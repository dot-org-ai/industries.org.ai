...
```typescript
import { operateMailboxServices } from '@headlessly/operate-mailbox-services'

const mailCenter = operateMailboxServices()

// Check available mailboxes and expiring rentals
const available = await mailCenter.findMailboxes({ status: 'vacant', size: 'medium' })
const expiring = await mailCenter.findExpiringRentals({ daysAhead: 30 })
