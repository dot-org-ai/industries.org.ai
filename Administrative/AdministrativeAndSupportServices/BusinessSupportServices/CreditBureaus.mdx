...
```typescript
import { reportConsumerCredit } from '@headlessly/report-consumer-credit'

const bureau = reportConsumerCredit()

// Look up consumer file and open disputes
const consumer = await bureau.findConsumer({ ssn: '***-**-1234' })
const disputes = await bureau.findDisputes({ status: 'open', consumerSSN: '***-**-1234' })
