...
```typescript
import { facilitateBusinessExchanges } from '@headlessly/facilitate-business-exchanges'

const support = facilitateBusinessExchanges()

// Check barter member balances and active campaigns
const balances = await support.getPointBalances({ program: 'barter', threshold: 1000 })
const campaigns = await support.getCampaigns({ status: 'active' })
