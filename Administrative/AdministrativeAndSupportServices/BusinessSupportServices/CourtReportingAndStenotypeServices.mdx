...
// Check reporter availability and pending transcripts
const reporters = await reporting.getReporters({ certification: 'RMR', available: '2024-04-15' })
const production = await reporting.trackProduction({ status: 'in-progress' })
