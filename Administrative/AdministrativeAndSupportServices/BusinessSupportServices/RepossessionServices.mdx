...
| RepoAgent | Locates and physically repossesses vehicles and equipment in the field |
...
```typescript
import { repossessCollateralAssets } from '@headlessly/repossess-collateral-assets'

const repo = repossessCollateralAssets()

// Check open recovery orders and available agents
const openOrders = await repo.findOrders({ status: 'active', clientId: 'auto-finance-corp' })
const agents = await repo.findAgents({ availability: 'on-duty', region: 'northeast' })
