...
| FederalEnvironmentalAgency | Federal environmental protection agency |
...
| mobilizationCompleted | Equipment and crew on-site |
...
graph LR
    ...
    R -->|reports to| FederalEnvironmentalAgency
...
// Review active sites and pending permit approvals
const activeSites = await remediation.findSites({ status: 'in-progress', media: 'groundwater' })
const pendingPermits = await remediation.getPermitStatus({ status: 'pending' })
