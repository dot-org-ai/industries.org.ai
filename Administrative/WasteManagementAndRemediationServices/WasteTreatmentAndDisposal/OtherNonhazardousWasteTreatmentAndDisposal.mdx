...
| CompostMaster | Manages windrow turning schedules, temperature monitoring, and curing of finished compost |
...
| temperatureDropped | Compost pile temperature has fallen below target range |
...
// Check active pile status and product inventory
const piles = await facility.getPileStatus({ phase: 'thermophilic' })
const productStock = await facility.getProductInventory({ grade: 'premium', available: true })
