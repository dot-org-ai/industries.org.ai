...
graph LR
    ...
    T -->|reports to| FederalEnvironmentalAgency
...
// Check stored waste inventory and permit status
const wasteInventory = await facility.getWasteInventory({ storageUnit: 'area-A', status: 'awaiting-treatment' })
const permitStatus = await facility.getPermitStatus({ expiringWithin: 90 })
