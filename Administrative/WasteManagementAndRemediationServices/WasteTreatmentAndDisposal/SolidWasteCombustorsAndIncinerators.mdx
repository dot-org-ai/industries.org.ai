...
| dailyCutoffReached | End-of-day processing period has been reached for reporting |
...
// Check daily throughput and emissions readings
const throughput = await facility.getThroughput({ date: '2024-04-01' })
const emissions = await facility.getEmissionsData({ unitId: 'combustor-1', period: 'last-24h' })
...
facility.dailyCutoffReached(async ({ date }) => {
