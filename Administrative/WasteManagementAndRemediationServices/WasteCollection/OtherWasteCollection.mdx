...
| containerReachedCapacity | Roll-off or dumpster has reached capacity and requires swap |
...
// Check scheduled jobs and container locations
const todayJobs = await waste.findJobs({ date: '2024-04-05', status: 'scheduled' })
const availableContainers = await waste.getContainers({ status: 'available', type: 'rollOff' })
...
waste.containerReachedCapacity(async ({ containerId, jobId }) => {
