...
| FederalEnvironmentalAgency | Federal environmental regulator |
...
| HazmatDriver | Holds CDL with hazmat endorsement to transport regulated waste over public roads |
...
graph LR
    ...
    H -->|reports to| FederalEnvironmentalAgency
...
// Check open manifests and permit expirations
const openManifests = await hazwaste.getManifests({ status: 'in-transit' })
const permits = await hazwaste.checkPermits({ expiringWithin: 90 })
