...
| GarbageTruckDriver | Operates automated or rear-load compaction trucks on curbside collection routes |
...
| vehicleBrokeDown | Truck experienced mechanical failure |
...
// Check Monday routes and open service requests
const mondayRoutes = await waste.getRoutes({ day: 'monday', zone: 'north-district' })
const openRequests = await waste.checkServiceRequests({ status: 'open', priority: 'high' })
...
waste.vehicleBrokeDown(async ({ vehicleId, routeId, location }) => {
