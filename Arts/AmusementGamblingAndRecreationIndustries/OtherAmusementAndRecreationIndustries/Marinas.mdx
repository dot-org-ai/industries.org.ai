...
| weatherWarningIssued | Weather warning issued for marina area |
...
// Check slip availability and occupancy
const slips = await marina.getSlips({ status: 'available', minLength: 30 })

// Review open service work orders
const tickets = await marina.getServiceTickets({ status: 'open' })
...
marina.weatherWarningIssued(async ({ severity, forecast, duration }) => {
