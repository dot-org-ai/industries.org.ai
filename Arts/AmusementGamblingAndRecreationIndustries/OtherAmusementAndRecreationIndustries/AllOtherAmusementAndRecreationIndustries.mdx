...
graph LR
    ...
    R -->|regulated by| LicensingAuthority
```
...
// List available activities and experiences
const activities = await facility.getActivities({ status: 'available' })

// Check upcoming reservations
const reservations = await facility.getReservations({ date: new Date() })
