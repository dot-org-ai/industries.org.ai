...
```typescript
import { hostWagers } from '@headlessly/host-wagers'

const facility = hostWagers()

// Check upcoming bingo schedule
const schedule = await facility.getBingoSchedule({ week: 'current' })

// Review regulatory compliance status
const compliance = await facility.getCompliance({ year: 2024 })
