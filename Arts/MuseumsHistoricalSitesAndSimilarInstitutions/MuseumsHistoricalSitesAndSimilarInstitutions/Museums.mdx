...
// Search permanent collection by criteria
const results = await museum.searchCollection({ period: 'impressionist', medium: 'oil' })

// Review current and upcoming exhibitions
const exhibitions = await museum.getExhibitions({ status: 'current' })
