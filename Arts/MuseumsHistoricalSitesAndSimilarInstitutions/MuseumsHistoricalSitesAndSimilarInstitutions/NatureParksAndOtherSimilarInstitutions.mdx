...
// Check trail conditions and access status
const trails = await park.getTrails({ status: 'open' })

// Review recent wildlife observations
const wildlife = await park.getWildlife({ species: 'bald-eagle', period: 'last-30-days' })
