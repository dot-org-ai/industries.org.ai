...
graph LR
    ...
    Z -->|partners with| EducationPartner
```
...
```typescript
import { exhibitAnimals } from '@headlessly/exhibit-animals'

const zoo = exhibitAnimals()

// Check breeding program status
const breeding = await zoo.getBreedingStatus({ species: 'red-panda' })

// Review animal medical records
const health = await zoo.getMedicalRecords({ animalId: 'panda-ming' })
