...
| careerMilestoneReached | Significant achievement reached |
...
// List represented clients with contract status
const clients = await agency.getClients({ status: 'active' })

// Search available engagements for client placement
const opportunities = await agency.findOpportunities({ type: 'film', budget: 'major' })
