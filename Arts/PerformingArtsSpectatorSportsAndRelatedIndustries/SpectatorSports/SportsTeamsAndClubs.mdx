...
| Scout | Evaluates amateur and professional talent for potential acquisition |
...
```typescript
import { competeGames } from '@headlessly/compete-games'

const team = competeGames()

// Search available free agents by position
const freeAgents = await team.findFreeAgents({ position: 'pitcher', minWAR: 2.0 })

// Check current injury report
const injuries = await team.getInjuryReport({ status: 'active' })
