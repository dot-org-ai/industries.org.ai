...
```typescript
import { promoteTouringEvents } from '@headlessly/promote-touring-events'

const promoter = promoteTouringEvents()

// Search available artists by genre and date
const talent = await promoter.findTalent({ genre: 'country', availableDate: '2024-09-15' })

// Search venues by location and capacity
const venues = await promoter.findVenues({ city: 'nashville', minCapacity: 3000 })
