# Industries Business-as-Code

This folder contains MDX definitions for ~1000 NAICS industries modeled as Business-as-Code APIs.

## Your Task

When assigned an industry file, fill in the TODO sections following the template below. Each industry should be a complete, programmable API definition.

## Template Reference

Use this SoybeanFarming example as your model:
...
```typescript
import { farmSoybeans } from '@headlessly/farm-soybeans'

const farm = farmSoybeans()
...
- action: `Farm`, object: `Soybeans` → `@headlessly/farm-soybeans`
- action: `Manufacture`, object: `Steel` → `@headlessly/manufacture-steel`

Keep action and object in **Title Case** to preserve acronyms (IT, HVAC, HMO, etc.)
...
- Do not invent placeholder data - use realistic industry terms
...
Curated industry data is in `_curation/industries.tsv` with columns:
- code, name, shortFileName, action, object
