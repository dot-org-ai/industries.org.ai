...
| rentalBecameOverdue | Media not returned by due date |
| lateFeeAssessed | Overdue charge added to account |
| reservationCreated | Title held for customer |
| reservationFulfilled | Reserved title available for pickup |
...
graph LR
    ...
    V -->|regulated by| ContentRatingBoard
    V -->|contracts| RecyclingService
```
...
// Search catalog by various criteria
const titles = await rental.findTitles({ genre: 'action', rating: 'PG-13' })

// List recently added titles
const newReleases = await rental.getNewReleases({ week: 'current' })
...
rental.reservationFulfilled(async ({ customerId, titleId, locationId }) => {
...
rental.rentalBecameOverdue(async ({ rentalId, customerId, daysLate }) => {
