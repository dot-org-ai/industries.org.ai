...
graph LR
    ...
    D -->|complies with| CMS
    D -->|contracts| BiomedicialTechnician
```
...
// List active rentals for patient
const getPatientEquipmentResult = await dme.getPatientEquipment({ status: 'active' })

// Search inventory by type and size
const findAvailableEquipmentResult = await dme.findAvailableEquipment({ status: 'active' })
