...
| lowInventoryDetected | Fleet inventory dropped below minimum threshold |
...
// Search trucks and trailers by size and location
const availableVehicles = await rental.findAvailableVehicles({ type: 'box-truck', location: 'downtown', date: '2024-04-01' })

// List bookings by date or location
const reservations = await rental.getReservations({ location: 'downtown', week: 'current' })
...
rental.lowInventoryDetected(async ({ locationId, vehicleType, currentCount }) => {
