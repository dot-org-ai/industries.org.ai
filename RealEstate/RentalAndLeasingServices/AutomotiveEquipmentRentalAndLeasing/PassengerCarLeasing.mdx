...
| paymentBecameDue | Monthly payment due date |
| paymentReceived | Lease payment processed |
| paymentBecameLate | Payment past due date |
| leaseMatured | Lease approaching end of term |
...
const getLeasePortfolioResult = await leasing.getLeasePortfolio({ status: 'active' })

// List leases ending within date range
const findMaturingLeasesResult = await leasing.findMaturingLeases({ status: 'active' })
...
leasing.leaseMatured(async ({ leaseId, lesseeId, maturityDate }) => {
