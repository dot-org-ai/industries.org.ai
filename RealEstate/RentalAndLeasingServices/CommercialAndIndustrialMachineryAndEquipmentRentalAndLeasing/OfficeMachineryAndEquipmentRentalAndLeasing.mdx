...
| supplyDepleted | Supply stock fell below threshold |
...
// List deployed equipment by customer
const getCustomerFleetResult = await lessor.getCustomerFleet({ status: 'active' })

// View print volumes and trends
const getUsageReportResult = await lessor.getUsageReport({ status: 'active' })
...
lessor.supplyDepleted(async ({ equipmentId, supplyType, remainingPercent }) => {
...
lessor.leaseExpirationNeared(async ({ leaseId, customerId, expirationDate }) => {
