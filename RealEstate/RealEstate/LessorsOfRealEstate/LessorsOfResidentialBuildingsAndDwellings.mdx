...
| rentBecameDue | Monthly rent payment is due |
| paymentReceived | Rent payment has been processed |
| paymentBecameLate | Rent payment is past due date |
...
```typescript
import { leaseApartments } from '@headlessly/lease-apartments'

const lessor = leaseApartments()

// List available units by property, size, or price
const vacantUnits = await lessor.findVacantUnits({ bedrooms: 2, maxRent: 2500 })

// Retrieve payment history for a tenant
const tenantLedger = await lessor.getTenantLedger({ tenantId: 'T-45231' })
...
lessor.paymentBecameLate(async ({ tenantId, amountDue, daysLate }) => {
  if (daysLate >= 5) {
...
lessor.leaseExpirationNeared(async ({ leaseId, tenantId, currentRent }) => {
