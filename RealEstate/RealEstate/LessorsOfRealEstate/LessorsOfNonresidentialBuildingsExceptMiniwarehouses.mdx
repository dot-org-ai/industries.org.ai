...
| leaseExpirationNeared | Lease term ending within notice period |
...
graph LR
    ...
    L -->|consults| EnvironmentalConsultant
```
...
```typescript
import { leaseCommercialSpace } from '@headlessly/lease-commercial-space'

const lessor = leaseCommercialSpace()

// List vacant suites by property, size, or rate
const findAvailableSpaceResult = await lessor.findAvailableSpace({ status: 'active' })

// Retrieve all tenants and lease terms for property
const getTenantRosterResult = await lessor.getTenantRoster({ status: 'active' })
...
lessor.leaseExpirationNeared(async ({ leaseId, tenantId, expirationDate }) => {
