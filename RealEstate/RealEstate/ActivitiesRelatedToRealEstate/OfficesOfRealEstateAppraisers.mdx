...
```typescript
import { appraiseProperties } from '@headlessly/appraise-properties'

const appraiser = appraiseProperties()

// Search sold properties by criteria
const findComparablesResult = await appraiser.findComparables({ address: '456 Oak Lane', radius: 1, months: 6 })

// Track appraisal order progress
const getOrderStatusResult = await appraiser.getOrderStatus({ status: 'active' })
