...
| documentsPrepared | Closing package prepared |
...
```typescript
import { facilitateTransactions } from '@headlessly/facilitate-transactions'

const escrow = facilitateTransactions()

// Track escrow account balance and activity
const getEscrowStatusResult = await escrow.getEscrowStatus({ status: 'active' })

// List pending closings by date
const findOpenTransactionsResult = await escrow.findOpenTransactions({ status: 'active' })
