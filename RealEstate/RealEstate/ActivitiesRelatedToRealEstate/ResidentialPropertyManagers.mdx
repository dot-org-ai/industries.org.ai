...
```typescript
import { overseeResidentialProperties } from '@headlessly/oversee-residential-properties'

const manager = overseeResidentialProperties()

// List all managed properties and units
const getPortfolioResult = await manager.getPortfolio({ status: 'active' })

// List available units across portfolio
const findVacanciesResult = await manager.findVacancies({ status: 'active' })
