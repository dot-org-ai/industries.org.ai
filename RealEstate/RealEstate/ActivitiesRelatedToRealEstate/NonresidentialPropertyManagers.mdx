...
| paymentBecameDelinquent | Tenant payment past due |
...
| certificateExpirationNeared | Insurance or compliance document neared expiration |
...
graph TD
    ...
    J --> K{Capital Needed?}
    K -->|Yes| L[planCapital]
    K -->|No| M{Lease Expiration Neared?}
    L --> M
    M -->|Yes| N[renewLease]
    M -->|No| C
    N --> C
...
```typescript
import { overseeCommercialProperties } from '@headlessly/oversee-commercial-properties'

const manager = overseeCommercialProperties()

// List all tenants and lease details
const getTenantRosterResult = await manager.getTenantRoster({ status: 'active' })

// Show receivables by aging bucket
const getARAgingReportResult = await manager.getARAgingReport({ status: 'active' })
...
manager.certificateExpirationNeared(async ({ tenantId, certificateType, expirationDate }) => {
