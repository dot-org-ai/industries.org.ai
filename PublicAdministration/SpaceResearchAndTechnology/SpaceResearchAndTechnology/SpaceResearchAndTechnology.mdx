...
// Search space flights by status or objective
const findMissionsResult = await space.findMissions({ status: 'active' })

// Retrieve vehicles by type or mission
const getSpacecraftResult = await space.getSpacecraft({ status: 'active' })
