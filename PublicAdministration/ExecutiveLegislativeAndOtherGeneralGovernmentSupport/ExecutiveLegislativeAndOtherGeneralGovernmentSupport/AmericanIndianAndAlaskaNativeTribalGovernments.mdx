...
| TribalHistoricPreservationOfficer | Protects sacred sites and cultural artifacts under federal trust law |
...
```typescript
import { governTribalNations } from '@headlessly/govern-tribal-nations'

const tribal = governTribalNations()

// Search enacted legislation by subject or date
const findTribalCodesResult = await tribal.findTribalCodes({ status: 'active' })

// Retrieve membership data by status or lineage
const getEnrollmentRecordsResult = await tribal.getEnrollmentRecords({ status: 'active' })
