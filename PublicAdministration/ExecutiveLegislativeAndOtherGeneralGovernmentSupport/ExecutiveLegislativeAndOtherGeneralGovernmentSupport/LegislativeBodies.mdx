...
// Search legislation by sponsor, subject, or status
const findBillsResult = await legislature.findBills({ status: 'active' })

// Retrieve voting history by member or bill
const getVoteRecordsResult = await legislature.getVoteRecords({ status: 'active' })
