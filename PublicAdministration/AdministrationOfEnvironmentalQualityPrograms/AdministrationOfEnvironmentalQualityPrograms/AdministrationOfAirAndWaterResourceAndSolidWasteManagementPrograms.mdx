...
| applicationReceived | New permit application submitted for review |
...
```typescript
import { administerPollution } from '@headlessly/administer-pollution'

const environmental = administerPollution()

// Search authorizations by facility or type
const findPermitsResult = await environmental.findPermits({ status: 'active' })

// Retrieve environmental measurements by location
const getMonitoringDataResult = await environmental.getMonitoringData({ status: 'active' })
