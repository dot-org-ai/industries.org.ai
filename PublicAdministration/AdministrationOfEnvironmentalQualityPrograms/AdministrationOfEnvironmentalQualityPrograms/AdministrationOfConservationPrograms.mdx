...
// Search land use authorizations by location or type
const findPermitsResult = await conservation.findPermits({ status: 'active' })

// Retrieve conservation restrictions by area or holder
const getEasementsResult = await conservation.getEasements({ status: 'active' })
