...
| RegionalSecurityOfficer | Manages security operations for embassy personnel and facilities abroad |
...
| crisisDetected | International crisis or security threat identified |
...
```typescript
import { conductDiplomacy } from '@headlessly/conduct-diplomacy'

const diplomacy = conductDiplomacy()

// Search diplomatic posts by country or type
const findMissionsResult = await diplomacy.findMissions({ status: 'active' })

// Retrieve agreements by nation or subject
const getTreatiesResult = await diplomacy.getTreaties({ status: 'active' })
