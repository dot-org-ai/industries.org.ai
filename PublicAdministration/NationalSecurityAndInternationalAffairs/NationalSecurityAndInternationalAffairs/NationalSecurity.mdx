...
```typescript
import { defendNation } from '@headlessly/defend-nation'

const defense = defendNation()

// Search formations by type, location, or status
const findUnitsResult = await defense.findUnits({ status: 'available' })

// Retrieve actions by region or timeframe
const getOperationsResult = await defense.getOperations({ status: 'active' })
