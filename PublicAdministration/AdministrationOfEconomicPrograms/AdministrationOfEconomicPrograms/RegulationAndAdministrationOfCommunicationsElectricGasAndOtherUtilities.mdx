...
// Search pricing proceedings by utility or status
const findRateCasesResult = await regulator.findRateCases({ status: 'active' })

// Retrieve rate schedules by utility or service
const getTariffsResult = await regulator.getTariffs({ status: 'active' })
