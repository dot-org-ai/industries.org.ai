...
// Search quality documents by product or grade
const findCertificatesResult = await agriculture.findCertificates({ status: 'active' })

// Retrieve facility reviews by result or date
const getInspectionsResult = await agriculture.getInspections({ status: 'active' })
