...
// Search responses by type, location, or date
const findIncidentsResult = await fire.findIncidents({ status: 'open', priority: 'high' })

// Retrieve vehicles by station or status
const getApparatusResult = await fire.getApparatus({ status: 'active' })
