...
// Search supervisions by offender, officer, or status
const findCasesResult = await supervision.findCases({ status: 'active' })

// Retrieve visit records by date or type
const getContactsResult = await supervision.getContacts({ status: 'active' })
