...
// Search proceedings by party, type, or status
const findCasesResult = await court.findCases({ status: 'active' })

// Retrieve court schedule by date or judge
const getDocketResult = await court.getDocket({ status: 'active' })
