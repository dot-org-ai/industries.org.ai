...
| reexaminationBecameDue | Tenant eligibility reexamination deadline approaching |
...
// Search housing requests by status or type
const findApplicationsResult = await housing.findApplications({ status: 'active' })

// Retrieve applicant queue by preference or date
const getWaitingListResult = await housing.getWaitingList({ status: 'active' })
...
housing.reexaminationBecameDue(async ({ tenantId, currentAssistance }) => {
