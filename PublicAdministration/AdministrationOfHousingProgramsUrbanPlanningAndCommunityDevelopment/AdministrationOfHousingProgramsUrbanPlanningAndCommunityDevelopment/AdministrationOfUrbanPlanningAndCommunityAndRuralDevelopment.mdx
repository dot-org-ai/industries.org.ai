...
| applicationReceived | Development or zoning application submitted |
...
// Search land use areas by type or location
const findZoningDistrictsResult = await planning.findZoningDistricts({ status: 'active' })

// Retrieve zoning exceptions by property or date
const getVariancesResult = await planning.getVariances({ status: 'active' })
