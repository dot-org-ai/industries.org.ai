...
```typescript
import { administerPublicBenefits } from '@headlessly/administer-public-benefits'

const services = administerPublicBenefits()

// Search benefit requests by status or type
const findApplicationsResult = await services.findApplications({ status: 'active' })

// Retrieve beneficiary records by program or status
const getCasesResult = await services.getCases({ status: 'active' })
