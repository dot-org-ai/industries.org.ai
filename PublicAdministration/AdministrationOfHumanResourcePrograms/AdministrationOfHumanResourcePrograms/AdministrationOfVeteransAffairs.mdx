...
// Search benefit requests by type or status
const findClaimsResult = await va.findClaims({ status: 'active' })

// Retrieve veteran profile by SSN or service number
const getVeteranRecordResult = await va.getVeteranRecord({ status: 'active' })
