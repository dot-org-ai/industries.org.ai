...
| ContactTracer | Identifies and notifies individuals exposed to communicable diseases |
...
// Search disease reports by type or location
const findCasesResult = await health.findCases({ status: 'active' })

// Retrieve facility reviews by result or date
const getInspectionsResult = await health.getInspections({ status: 'active' })
