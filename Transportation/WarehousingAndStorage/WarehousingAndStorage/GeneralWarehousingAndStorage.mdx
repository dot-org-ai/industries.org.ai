...
| monthEnded | Monthly billing and reporting period closed |
...
const capacity = await warehouse.getCapacity({ zone: 'dry-storage', minPallets: 200 })

// Find items below reorder point
const lowStock = await warehouse.getLowStock({ clientId: 'client-abc', threshold: 'reorder-point' })
...
warehouse.monthEnded(async ({ month }) => {
