...
| payPeriodEnded | Pay period closed and wages calculated |
| quarterEnded | Quarterly tax filing period reached |
...
const findEmployeesResult = await household.findEmployees({ status: 'active' })

// View work hours
const getSchedulesResult = await household.getSchedules({ status: 'active' })
...
household.payPeriodEnded(async ({ periodEnd, employees }) => {
...
household.quarterEnded(async ({ quarter, year }) => {
