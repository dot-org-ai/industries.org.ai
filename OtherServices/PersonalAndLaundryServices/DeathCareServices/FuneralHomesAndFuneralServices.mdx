...
```typescript
import { interDeceased } from '@headlessly/inter-deceased'

const funeralHome = interDeceased()

// Browse available caskets and prices
const findCasketOptionsResult = await funeralHome.findCasketOptions({ status: 'active' })

// View funeral service options
const getServicePackagesResult = await funeralHome.getServicePackages({ status: 'active' })
