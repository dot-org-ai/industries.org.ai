...
```typescript
import { launderLinens } from '@headlessly/launder-linens'

const supplier = launderLinens()

// View delivery routes and times
const getRouteScheduleResult = await supplier.getRouteSchedule({ status: 'active' })

// Monitor linen stock by type
const checkInventoryLevelsResult = await supplier.checkInventoryLevels({ status: 'active' })
