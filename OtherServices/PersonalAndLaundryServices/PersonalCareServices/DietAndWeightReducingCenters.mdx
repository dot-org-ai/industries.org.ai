...
| BodyCompositionAnalyst | Performs bioelectrical impedance and measurement assessments |
...
// Retrieve weight and measurement history
const getClientProgressResult = await center.getClientProgress({ status: 'active' })

// Search available weight loss plans
const findProgramOptionsResult = await center.findProgramOptions({ status: 'active' })
