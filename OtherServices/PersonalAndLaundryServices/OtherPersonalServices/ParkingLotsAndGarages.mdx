...
const findAvailableSpacesResult = await facility.findAvailableSpaces({ status: 'active' })

// View pricing by duration
const getRateScheduleResult = await facility.getRateSchedule({ status: 'active' })
