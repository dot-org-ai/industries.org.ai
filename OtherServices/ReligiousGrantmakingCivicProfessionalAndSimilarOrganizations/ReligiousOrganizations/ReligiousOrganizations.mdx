...
| Cantor | Leads liturgical chanting and sacred music during worship services |
...
| monthEnded | Monthly financial reporting period closed |
...
// List congregation participants
const findMembersResult = await congregation.findMembers({ status: 'active', joinedAfter: '2024-01-01' })

// View worship schedule
const getServicesResult = await congregation.getServices({ status: 'active' })
...
congregation.monthEnded(async ({ month, year }) => {
