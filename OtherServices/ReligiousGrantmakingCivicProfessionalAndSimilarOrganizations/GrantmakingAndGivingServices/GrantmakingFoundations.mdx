...
| disbursementNeared | Scheduled grant payment date nearing |
...
const findApplicationsResult = await foundation.findApplications({ status: 'active' })

// View active awards
const getGrantsResult = await foundation.getGrants({ status: 'open', deadline: 'next-60-days' })
...
foundation.disbursementNeared(async ({ grantId, disbursementId, dueDate }) => {
