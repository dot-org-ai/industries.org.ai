...
| researchMilestoneReached | Funded research project reached key milestone |
...
```typescript
import { grantHealthResearch } from '@headlessly/grant-health-research'

const healthOrg = grantHealthResearch()

// List contributors
const findDonorsResult = await healthOrg.findDonors({ minGift: 1000, period: 'last-12-months' })

// View fundraising initiatives
const getCampaignsResult = await healthOrg.getCampaigns({ status: 'active', year: 2024 })
