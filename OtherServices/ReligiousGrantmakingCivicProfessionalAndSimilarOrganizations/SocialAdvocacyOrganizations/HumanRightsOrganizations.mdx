...
| legislationIntroduced | New legislation relevant to human rights introduced |
| caseDecided | Court ruling issued on human rights case |
...
// List supporters
const findMembersResult = await rightsOrg.findMembers({ status: 'active', joinedAfter: '2024-01-01' })

// View legal matters
const getCasesResult = await rightsOrg.getCases({ status: 'active' })
