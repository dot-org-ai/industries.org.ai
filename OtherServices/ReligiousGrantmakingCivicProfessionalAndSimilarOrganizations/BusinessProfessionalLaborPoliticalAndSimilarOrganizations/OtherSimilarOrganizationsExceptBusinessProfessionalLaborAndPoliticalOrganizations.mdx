...
| assessmentOverdue | Member assessment payment past due |
| reserveComponentDue | Reserve fund contribution payment due |
...
hoa.assessmentOverdue(async ({ memberId, amount, daysOverdue }) => {
  if (daysOverdue === 30) {
...
hoa.reserveComponentDue(async ({ componentId, replacementCost, dueYear }) => {
