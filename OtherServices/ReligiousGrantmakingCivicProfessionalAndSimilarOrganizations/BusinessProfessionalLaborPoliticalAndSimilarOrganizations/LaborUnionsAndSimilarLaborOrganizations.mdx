...
| grievanceDeadlinePassed | Grievance response deadline expired |
| contractExpirationNeared | Collective bargaining agreement nearing expiration |
...
// List worker participants
const findMembersResult = await union.findMembers({ status: 'active', joinedAfter: '2024-01-01' })

// View collective agreements
const getContractsResult = await union.getContracts({ status: 'active', department: 'public-works' })
...
union.contractExpirationNeared(async ({ contractId, expirationDate, daysRemaining }) => {
  if (daysRemaining === 90) {
