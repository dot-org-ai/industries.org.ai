...
| credentialExpirationNeared | Member credential renewal deadline nearing |
...
association.credentialExpirationNeared(async ({ memberId, credentialId, expirationDate }) => {
  await association.notifyMember({
    memberId,
