...
```typescript
import { repairGlass } from '@headlessly/repair-glass'

const shop = repairGlass()

// Look up part number and features for vehicle
const findGlassSpecResult = await shop.findGlassSpec({ status: 'active' })

// Search available glass in stock
