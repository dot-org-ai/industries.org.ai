...
// Look up manufacturer paint formula
const findPaintCodeResult = await shop.findPaintCode({ status: 'active' })

// Check OEM and aftermarket parts stock
const searchPartsAvailabilityResult = await shop.searchPartsAvailability({ status: 'active' })
