...
```typescript
import { repairAutoAccessories } from '@headlessly/repair-auto-accessories'

const shop = repairAutoAccessories()

// Look up correct tire specifications for vehicle
const findTireSizeResult = await shop.findTireSize({ status: 'active' })

// Search available tires in stock
const checkTireInventoryResult = await shop.checkTireInventory({ status: 'active' })
