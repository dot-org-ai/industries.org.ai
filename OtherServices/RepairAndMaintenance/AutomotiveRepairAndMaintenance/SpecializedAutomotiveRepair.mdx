...
```typescript
import { repairDrivetrains } from '@headlessly/repair-drivetrains'

const shop = repairDrivetrains()

// Look up meaning and solutions for trouble codes
const findDiagnosticCodesResult = await shop.findDiagnosticCodes({ status: 'active' })

// Retrieve manufacturer service advisories
const getTechnicalBulletinsResult = await shop.getTechnicalBulletins({ status: 'active' })
