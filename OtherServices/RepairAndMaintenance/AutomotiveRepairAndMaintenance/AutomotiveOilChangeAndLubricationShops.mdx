...
// Look up recommended oil grade for vehicle
const findOilSpecResult = await shop.findOilSpec({ status: 'active' })

// Retrieve manufacturer maintenance schedule
const getServiceIntervalResult = await shop.getServiceInterval({ status: 'active' })
