...
// Retrieve all service records for a vehicle
const findVehicleHistoryResult = await shop.findVehicleHistory({ vehicleId: 'veh-456' })

// List all in-progress repair orders
const getOpenRepairOrdersResult = await shop.getOpenRepairOrders({ status: 'in-progress' })
