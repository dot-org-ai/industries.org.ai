...
| itemPrepared | Goods prepared for pickup |
...
// Look up correct replacement component
const findPartNumberResult = await shop.findPartNumber({ status: 'active' })

// Identify movement type for service
const getWatchMovementResult = await shop.getWatchMovement({ status: 'active' })
