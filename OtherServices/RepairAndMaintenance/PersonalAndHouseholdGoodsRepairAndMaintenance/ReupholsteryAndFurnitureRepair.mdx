...
// Search available upholstery materials
const findFabricOptionsResult = await shop.findFabricOptions({ status: 'active' })

// Look up appropriate cushion firmness
const getFoamDensityResult = await shop.getFoamDensity({ status: 'active' })
