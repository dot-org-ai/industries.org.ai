...
| Roaster | Roasts, cups, and profiles coffee beans for freshness and flavor |
...
graph LR
    ...
    S -->|inspected by| HealthDepartment
```
...
// Check pending mobile orders and inventory levels
const mobileQueue = await cafe.getMobileQueue()
const stock = await cafe.getInventoryLevels({ category: 'milk-alternatives' })
...
cafe.inventoryDepleted(async ({ item, currentLevel, usageRate }) => {
