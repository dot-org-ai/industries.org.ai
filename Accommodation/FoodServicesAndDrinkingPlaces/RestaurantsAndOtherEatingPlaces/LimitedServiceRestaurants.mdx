...
| orderPrepared | Complete order available for pickup |
...
| inventoryThresholdReached | Inventory level crossed reorder threshold |
...
graph LR
    ...
    R -->|buys from| KitchenEquipmentVendor
```
...
// Check pending delivery orders and current inventory
const deliveries = await restaurant.getPendingDeliveries()
const stock = await restaurant.getInventoryLevels({ category: 'proteins' })
...
restaurant.inventoryThresholdReached(async ({ item, currentLevel, parLevel }) => {
