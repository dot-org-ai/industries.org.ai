...
| temperatureExceeded | Food outside safe range detected |
...
```typescript
import { serveBuffets } from '@headlessly/serve-buffets'

const cafeteria = serveBuffets()

// Check station status and inventory before lunch service
const stations = await cafeteria.getStationStatus()
const inventory = await cafeteria.getInventoryLevels({ mealPeriod: 'lunch' })
...
cafeteria.temperatureChecked(async ({ station, readings }) => {
  ...
  if (violations.length > 0) {
    await cafeteria.temperatureExceeded({
      station,
      violations,
