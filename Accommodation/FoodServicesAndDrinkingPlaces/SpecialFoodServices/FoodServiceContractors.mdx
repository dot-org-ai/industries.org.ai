...
| Event | Description |
|-------|-------------|
| contractBidSubmitted | Proposal submitted for consideration |
...
```typescript
import { caterInstitutions } from '@headlessly/cater-institutions'

const contractor = caterInstitutions()

// Review active contracts and compliance status
const activeContracts = await contractor.getContracts({ status: 'active' })
const compliance = await contractor.getComplianceStatus({ locationId: 'techcorp-building-a' })
