...
| CasinoHost | Cultivates high-value player relationships and arranges complimentary services |
...
| highValueDetected | Unusual betting pattern detected by surveillance |
...
```typescript
import { lodgeCasinoGuests } from '@headlessly/lodge-casino-guests'

const casino = lodgeCasinoGuests()

// Look up high-value players arriving this weekend
const vips = await casino.findHighValuePlayers({ minTheoWin: 5000, period: 'last-90-days' })

// Check slot floor performance for rebalancing
const slotStats = await casino.getSlotPerformance({ zone: 'main-floor', metric: 'hold-percentage' })
