...
| HopsPicker | Operates hop picking machine to separate cones from bines during harvest |
...
| harvestReadied | Crop reached optimal stage |
...
// Check certification status for organic compliance
const certs = await farm.getCertifications({ fieldId: 'hop-yard-1', type: 'organic' })

// Find specialty buyers with current bids
const buyers = await farm.findBuyers({ crop: 'hops', market: 'craft-brewery' })
