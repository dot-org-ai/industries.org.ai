...
| Defoliator | Times and applies defoliant chemicals to prepare plants for mechanical picking |
...
// Check gin availability and wait times
const ginSchedule = await farm.getGinSchedule({ region: 'south-plains' })

// Get regional pest pressure advisories
const pestAlerts = await farm.getPestPressure({ region: 'south-plains', pests: ['bollworm', 'plantBugs'] })
