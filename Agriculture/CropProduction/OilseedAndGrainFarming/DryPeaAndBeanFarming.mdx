...
// Check disease alerts for pulse crops in the region
const alerts = await farm.getDiseaseAlerts({ region: 'northern-plains', crop: 'lentil' })

// Find fields with favorable pulse rotation history
const fields = await farm.findFields({ pulseHistory: 'none-3yr', diseaseRisk: 'low' })
