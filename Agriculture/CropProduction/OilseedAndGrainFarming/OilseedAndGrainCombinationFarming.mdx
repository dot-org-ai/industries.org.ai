...
| RotationPlanner | Designs multi-year crop sequences to optimize soil health and profitability |
...
// Get current prices across all rotation crops
const prices = await farm.getPrices({ crops: ['corn', 'soybeans', 'wheat'] })

// Review rotation benefit data for yield impact
const benefit = await farm.getRotationBenefit({ field: 'north-80', years: 5 })
