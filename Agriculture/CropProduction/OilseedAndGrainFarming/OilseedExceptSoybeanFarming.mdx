...
// Check premiums for high-oleic sunflower varieties
const premiums = await farm.getOilPremiums({ crop: 'sunflower', trait: 'high-oleic' })

// Find fields with suitable canola rotation history
const fields = await farm.findFields({ cropHistory: 'no-canola-3yr', soilType: 'clay-loam' })
