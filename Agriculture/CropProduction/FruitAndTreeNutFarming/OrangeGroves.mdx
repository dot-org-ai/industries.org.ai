...
| frostWarningIssued | Freeze event forecast for region |
| frostProtected | Frost protection measures activated |
| fruitSampled | Brix and maturity test completed |
| harvestReadied | Fruit reached optimal maturity |
...
// Check greening infection levels across the grove
const greening = await grove.getGreeningStatus({ groveId: 'main-grove' })

// Get frost risk probability for the grove location
const frostRisk = await grove.getFrostRisk({ location: 'indian-river' })
...
grove.frostWarningIssued(async ({ blocks, forecastLow }) => {
  if (forecastLow <= 28) {
    await grove.protectFrost({
      blocks,
      method: 'wind-machine',
