...
| harvestReadied | Fruit reached optimal ripeness |
...
// Check current market prices by grade
const prices = await farm.getPrice({ grade: 'Fancy', market: 'retail' })

// Review projected harvest volume schedule
const schedule = await farm.getHarvestSchedule({ fieldId: 'block-a', weeks: 4 })
...
farm.frostWarningIssued(async ({ fields, forecastLow }) => {
  if (forecastLow <= 32) {
