...
| frostWarningIssued | Freeze event forecast |
...
// Check maturity status across blocks
const maturity = await grove.getMaturityStatus({ variety: 'ruby-red' })

// Get current prices by citrus type and grade
const pricing = await grove.getPrice({ variety: 'grapefruit', grade: 'Fancy' })
...
grove.frostWarningIssued(async ({ blocks, forecastLow, duration }) => {
