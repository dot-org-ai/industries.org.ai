...
| budBroke | Buds beginning to open |
...
| veraisonOccurred | Berries beginning color change |
...
graph LR
    ...
    G -->|processes via| RaisinProcessor
```
...
```typescript
import { growGrapes } from '@headlessly/grow-grapes'

const vineyard = growGrapes()

// Find blocks by variety and appellation
const blocks = await vineyard.findBlocks({ variety: 'cabernet-sauvignon', appellation: 'napa-valley' })

// Get optimal harvest window based on maturity data
const harvestWindow = await vineyard.getHarvestWindow({ blockId: 'cabernet-block-7' })
