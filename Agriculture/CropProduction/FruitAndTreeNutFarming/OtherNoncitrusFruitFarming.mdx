...
| chillHoursUpdated | Accumulated chill hour count has been recalculated for the region |
...
// Check accumulated chill hours for the region
const chill = await orchard.getChillHours({ region: 'san-joaquin-valley' })

// Get optimal harvest window for peaches
const window = await orchard.getMaturityWindow({ blockId: 'elberta-5', fruitType: 'peach' })
