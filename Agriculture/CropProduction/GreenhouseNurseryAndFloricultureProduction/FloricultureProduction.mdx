...
| harvestReadied | Flowers reached target stage |
...
// Check upcoming holiday demand calendar
const holidays = await greenhouse.getHolidayCalendar({ months: 6 })

// Get pending orders for production planning
const orders = await greenhouse.getOrders({ status: 'confirmed', weeks: 8 })
