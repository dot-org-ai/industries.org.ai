...
// Check propagation status for current crops
const propStatus = await nursery.getPropagationStatus({ species: 'acer-rubrum' })

// Get pending orders to prioritize production
const pendingOrders = await nursery.getOrders({ status: 'pending' })
