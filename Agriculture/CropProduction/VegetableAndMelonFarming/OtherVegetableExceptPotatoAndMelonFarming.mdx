...
| TransplantGrower | Raises seedlings in plug trays for field transplanting |
...
| harvestReadied | Crop reached optimal maturity |
...
// Check current wholesale prices for target crops
const prices = await farm.getPrice({ crops: ['tomato', 'pepper', 'watermelon'] })

// Find fields with drip irrigation for vegetable production
const fields = await farm.findFields({ irrigationType: 'drip', cropHistory: 'rotation-ready' })
...
farm.harvestReadied(async ({ fieldId, crop, maturityIndex }) => {
