...
| weatherWarningIssued | Weather warning issued for fishing area |
...
// Check dive conditions for harvest area
const conditions = await fishing.getDiveConditions({ area: 'channel-islands-zone-3' })

// Review remaining quota balance
const quota = await fishing.getQuotaBalance({ species: 'red-sea-urchin' })
...
fishing.weatherWarningIssued(async ({ area, conditions, severity }) => {
