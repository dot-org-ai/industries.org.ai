...
| HoofTrimmer | Trims and corrects dairy cattle hooves to prevent lameness |
...
```typescript
import { deliverLivestockServices } from '@headlessly/deliver-livestock-services'

const services = deliverLivestockServices()

// View upcoming service schedule
const schedule = await services.getSchedule({ week: 'current' })

// List active livestock clients
const clients = await services.findClients({ serviceType: 'artificial-insemination' })
