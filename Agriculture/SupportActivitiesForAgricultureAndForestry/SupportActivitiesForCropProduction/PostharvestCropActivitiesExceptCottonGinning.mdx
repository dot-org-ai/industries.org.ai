...
| PrecoolingOperator | Operates forced-air and hydrocooling systems to quickly remove field heat |
...
```typescript
import { prepareHarvestedCrops } from '@headlessly/prepare-harvested-crops'

const facility = prepareHarvestedCrops()

// Check available processing and cold storage capacity
const capacity = await facility.getCapacity({ date: new Date() })

// Get current market prices by grade for pricing
const prices = await facility.getPriceByGrade({ crop: 'apples', variety: 'honeycrisp' })
