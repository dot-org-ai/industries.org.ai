...
| seasonCompleted | Harvest ginning finished |
| equipmentMaintenancePerformed | Scheduled service completed |
...
// Check bale inventory and farmer deliveries
const inventory = await gin.getBaleInventory({ status: 'unginned' })

// Get current lint prices by grade
const prices = await gin.getPricingHistory({ season: '2024', grades: ['41-4', '31-3'] })
