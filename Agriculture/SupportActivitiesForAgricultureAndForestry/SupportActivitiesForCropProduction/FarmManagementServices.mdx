...
| LandAppraiser | Values agricultural real estate and assesses productivity potential |
...
```typescript
import { overseeFarmOperations } from '@headlessly/oversee-farm-operations'

const manager = overseeFarmOperations()

// Review portfolio of farms under management
const portfolio = await manager.getPortfolio({ status: 'active' })

// Check current commodity prices for planning
const prices = await manager.getMarketPrices({ crops: ['cabernet-sauvignon', 'merlot'] })
