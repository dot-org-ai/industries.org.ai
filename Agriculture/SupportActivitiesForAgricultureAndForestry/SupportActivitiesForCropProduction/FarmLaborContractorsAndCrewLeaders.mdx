...
| H2ACoordinator | Manages visa petitions, consulate appointments, and worker arrival logistics |
...
// Check available workers by skill set
const workers = await contractor.getWorkerRoster({ skill: 'tree-fruit-picking', status: 'available' })

// Review current crew deployments
const crews = await contractor.getCrewAssignments({ week: 'current' })
