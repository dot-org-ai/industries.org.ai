...
| inventoryCompleted | Timber cruise finished |
...
| harvestCompleted | Timber removal finished |
...
```typescript
import { stewardTimberland } from '@headlessly/steward-timberland'

const forest = stewardTimberland()

// Review tract inventory and standing volumes
const tracts = await forest.getTractInventory({ minAcres: 20 })

// Check sustainable forestry certification status
const cert = await forest.getCertificationStatus({ tractId: 'tract-north-40' })
...
forest.harvestCompleted(async ({ harvestUnitId, completionDate }) => {
