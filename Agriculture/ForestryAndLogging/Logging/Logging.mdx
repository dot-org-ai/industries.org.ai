...
| loadingCompleted | Truck filled and ready |
| loadDelivered | Logs arrived at mill |
| loadScaled | Volume measured and recorded |
| harvestCompleted | All timber removed from unit |
| contractClosed | Final settlement complete |
| equipmentBrokeDown | Machine failure requiring repair |
| weatherDelayed | Operations suspended for conditions |
...
// Find available stumpage sales to bid on
const sales = await logging.findStumpage({ region: 'southeast', species: 'pine' })

// Check current mill prices by log grade
const prices = await logging.getMillPrices({ product: 'sawlog', species: 'pine' })
...
logging.loadingCompleted(async ({ landingId, truckId, product, volume }) => {
...
logging.equipmentBrokeDown(async ({ machineId, issue, severity }) => {
