...
| SeineOperator | Deploys and retrieves seine nets during pond harvest |
...
| feedingCompleted | Daily feeding cycle finished |
...
| harvestCompleted | Fish removed and loaded for transport |
...
// Check pond inventory by species and size class
const inventory = await farm.getPondInventory({ species: 'channel-catfish' })

// Get water quality readings across ponds
const water = await farm.getWaterQualityLogs({ pondId: 'grow-pond-3' })
...
farm.waterQualityDegraded(async ({ pondId, parameter, value, threshold }) => {
