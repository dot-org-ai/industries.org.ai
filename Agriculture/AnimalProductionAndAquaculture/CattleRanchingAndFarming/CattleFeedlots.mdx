...
```typescript
import { raiseFeederCattle } from '@headlessly/raise-feeder-cattle'

const feedlot = raiseFeederCattle()

// Check current cash and grid prices at packers
const prices = await feedlot.getPrice({ grade: 'choice', yieldGrade: 3 })

// Review ration ingredient costs
const rationCosts = await feedlot.getRationCosts({ stage: 'finisher' })
