...
| broodingCompleted | Birds transitioned from heat lamps |
...
| mortalityThresholdExceeded | Flock mortality rate exceeded acceptable threshold |
| condemnationThresholdExceeded | Condemnation rate exceeded acceptable threshold |
| houseBecameVacant | Flock removed and downtime begins |
...
farm.mortalityThresholdExceeded(async ({ houseId, dailyRate, cause }) => {
  if (dailyRate > 0.5) {
