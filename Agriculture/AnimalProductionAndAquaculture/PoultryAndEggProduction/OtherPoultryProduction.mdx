...
| birdsMarketPrepared | Flock reached target weight or age |
...
// Check permit and license status
const permits = await farm.getPermitStatus({ species: 'pheasant' })

// Get current specialty poultry market prices
const prices = await farm.getMarketPrices({ species: 'pheasant', product: 'live-birds' })
...
farm.birdsMarketPrepared(async ({ species, quantity, avgWeight, penId }) => {
