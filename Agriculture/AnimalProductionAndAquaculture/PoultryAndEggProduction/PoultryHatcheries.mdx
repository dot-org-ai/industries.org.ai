...
| candlingCompleted | Fertility check finished, clears removed |
...
| biosecurityBreachDetected | Biosecurity protocol breach detected |
...
// Check upcoming hatch schedule and expected pull dates
const schedule = await hatchery.getHatchSchedule({ weeks: 4 })

// Review egg inventory by breeder source
const eggs = await hatchery.trackEggInventory({ maxAge: 7 })
...
hatchery.candlingCompleted(async ({ hatchId, fertilityRate, viableEggs }) => {
