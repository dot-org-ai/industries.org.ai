...
| Event | Description |
|-------|-------------|
| breedingComplete | Seasonal breeding finished |
| kindled | Does have given birth |
| kitsWeaned | Young separated from mothers |
| primingComplete | Fur coat at peak condition |
| harvestComplete | Animals processed |
...
| regulatoryInspection | Compliance audit completed |
...
farm.primingComplete(async ({ colonyId, species, avgFurScore }) => {
  if (avgFurScore >= 8.5) {
