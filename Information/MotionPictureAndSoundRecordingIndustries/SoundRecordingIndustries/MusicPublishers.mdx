...
```typescript
import { publishSongs } from '@headlessly/publish-songs'

const publisher = publishSongs()

// Review catalog and pending sync opportunities
const catalog = await publisher.searchCatalog({ genre: 'indie-pop', tempo: 'upbeat' })
const syncOpps = await publisher.getSyncOpportunities({ status: 'pending', media: 'film' })
