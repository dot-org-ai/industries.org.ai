...
```typescript
import { recordAudioMasters } from '@headlessly/record-audio-masters'

const recordingService = recordAudioMasters()

// Check upcoming schedule and available equipment
const schedule = await recordingService.getSchedule({ week: '2025-03-20' })
const equipment = await recordingService.getEquipment({ status: 'available', type: 'digital-recorder' })
