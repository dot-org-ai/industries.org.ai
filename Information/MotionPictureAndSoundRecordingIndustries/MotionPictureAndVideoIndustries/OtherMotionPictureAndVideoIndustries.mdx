...
```typescript
import { archiveFilmAssets } from '@headlessly/archive-film-assets'

const archive = archiveFilmAssets()

// Search existing holdings and check vault capacity
const existing = await archive.searchFootage({ subject: 'lost-horizon', era: '1930s' })
const vault = await archive.getVaultInventory({ location: 'cold-storage-a', format: '35mm' })
