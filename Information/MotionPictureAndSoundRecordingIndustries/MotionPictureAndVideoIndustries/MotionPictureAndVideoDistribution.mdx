...
```typescript
import { distributeFilms } from '@headlessly/distribute-films'

const distribution = distributeFilms()

// Review release schedule and available rights
const releaseSchedule = await distribution.getReleaseSchedule({ territory: 'north-america', month: '2025-07' })
const rights = await distribution.getRights({ titleId: 'film-456', territory: 'north-america' })
