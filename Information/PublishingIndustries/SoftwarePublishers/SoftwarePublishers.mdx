...
// Review open bugs and subscription metrics
const bugs = await publisher.getBugs({ product: 'prod-analytics', severity: 'critical', status: 'open' })
const subscriptions = await publisher.getSubscriptions({ plan: 'enterprise', status: 'active' })
