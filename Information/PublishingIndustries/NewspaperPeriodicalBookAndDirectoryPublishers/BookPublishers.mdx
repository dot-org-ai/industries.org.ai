...
```typescript
import { publishBooks } from '@headlessly/publish-books'

const publisher = publishBooks()

// Review manuscript submissions and publication schedule
const manuscripts = await publisher.getManuscripts({ status: 'under-review', genre: 'literary-fiction' })
const schedule = await publisher.getSchedule({ quarter: '2025-Q4' })
