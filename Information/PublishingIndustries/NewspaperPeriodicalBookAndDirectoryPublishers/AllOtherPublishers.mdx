...
| EditorialDirector | Sets content strategy and oversees all publications |
...
```typescript
import { publishPublications } from '@headlessly/publish-publications'

const publisher = publishPublications()

// Review catalog and inventory levels
const catalog = await publisher.searchCatalog({ type: 'wall-calendar', year: 2026 })
const inventory = await publisher.getInventory({ warehouse: 'main', belowReorder: true })
