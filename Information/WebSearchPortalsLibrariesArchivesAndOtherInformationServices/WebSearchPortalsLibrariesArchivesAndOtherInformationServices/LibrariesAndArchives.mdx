...
// Check current holds and overdue items
const holds = await library.getHolds({ status: 'pending', branch: 'main' })
const overdues = await library.getOverdues({ daysOverdue: 14 })
