...
| satelliteBecameOperational | Service commenced |
...
```typescript
import { beamSatelliteTelecommunications } from '@headlessly/beam-satellite-telecommunications'

const satop = beamSatelliteTelecommunications()

// Check satellite fleet health and available capacity
const satellites = await satop.getSatellites({ status: 'operational' })
const availableCapacity = await satop.getCapacity({ satelliteId: 'sat-galaxy-19', beamId: 'north-america' })
