...
| CapacityBroker | Negotiates wholesale bandwidth and circuit purchases |
...
```typescript
import { connectBandwidth } from '@headlessly/connect-bandwidth'

const mvno = connectBandwidth()

// Review subscriber MRR and churn risk
const mrr = await mvno.getMRR({ period: '2025-03', segment: 'consumer' })
const atRisk = await mvno.getChurn({ riskAbove: 0.6 })
