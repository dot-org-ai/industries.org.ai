...
```typescript
import { connectWirelessPlans } from '@headlessly/connect-wireless-plans'

const agent = connectWirelessPlans()

// Check inventory and sales quota progress
const inventory = await agent.getInventory({ storeId: 'store-mall-central', category: 'iphone' })
const quotas = await agent.getQuotas({ associateId: 'assoc-smith', period: '2025-03' })
