...
```typescript
import { streamMediaStreaming } from '@headlessly/stream-media-streaming'

const platform = streamMediaStreaming()

// Review catalog and expiring licenses
const catalog = await platform.searchCatalog({ genre: 'drama', region: 'north-america' })
const licenses = await platform.getLicenses({ expiresWithin: '90-days' })
