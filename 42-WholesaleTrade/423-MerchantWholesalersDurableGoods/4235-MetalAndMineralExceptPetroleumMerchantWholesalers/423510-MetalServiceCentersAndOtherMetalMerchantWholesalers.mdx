...
| remnantAvailable | Leftover material for sale |
...
```typescript
import { wholesaleMetalService } from '@headlessly/wholesale-metal-service'

const wholesaler = wholesaleMetalService()
...
wholesaler.remnantAvailable(async ({ remnantId, metal, size }) => {
  await listRemnant({
    remnantId,
    discount: 30,
