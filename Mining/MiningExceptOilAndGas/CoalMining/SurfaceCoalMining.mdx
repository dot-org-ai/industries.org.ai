...
| DraglineOperator | Operates walking draglines to remove overburden and expose coal seams |
...
| equipmentWentDown | Major equipment requires unplanned maintenance |
...
// Check coal stockpile levels and available grades
const inventory = await mine.getInventory({ grade: 'thermal', minBTU: 8800 })

// Find potential buyers with open contract needs
const buyers = await mine.findBuyers({ coalType: 'thermal', region: 'midwest' })
...
mine.safetyIncidentOccurred(async ({ type, severity, location }) => {
