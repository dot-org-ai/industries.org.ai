...
| ShuttleCarOperator | Transports cut coal from working face to conveyor belt |
...
| equipmentWentDown | Mining equipment requires repair |
...
// Check ventilation readings in active panels
const ventilation = await mine.getVentilation({ section: 'mains', includeGasReadings: true })

// Review active panel production and reserves
const panels = await mine.findPanels({ status: 'active', minReserves: 500000 })
...
mine.equipmentWentDown(async ({ equipmentId, type, location }) => {
