...
```typescript
import { quarryLimestoneAggregate } from '@headlessly/quarry-limestone-aggregate'

const quarry = quarryLimestoneAggregate()

// Check current stockpile levels by product size
const inventory = await quarry.getInventory({ product: 'crushed-limestone', sizeRange: '57-stone' })

// Get current aggregate prices
const prices = await quarry.checkPrices({ product: 'base-aggregate', region: 'indiana' })
