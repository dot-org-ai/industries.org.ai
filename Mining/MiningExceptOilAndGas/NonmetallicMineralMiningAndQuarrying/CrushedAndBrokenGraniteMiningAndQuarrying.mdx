...
| DrillRigOperator | Operates track-mounted drills to bore blast holes in granite benches |
...
```typescript
import { quarryGraniteAggregate } from '@headlessly/quarry-granite-aggregate'

const quarry = quarryGraniteAggregate()

// Check current aggregate prices
const prices = await quarry.checkPrices({ product: 'granite-base', region: 'southeast' })

// Find buyers needing granite aggregate
const customers = await quarry.findCustomers({ productType: 'crushed-granite', openOrders: true })
