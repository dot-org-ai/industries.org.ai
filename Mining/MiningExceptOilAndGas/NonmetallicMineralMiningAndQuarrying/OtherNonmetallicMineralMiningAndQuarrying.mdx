...
// Check current mineral prices
const prices = await mine.checkPrices({ mineral: 'phosphate-rock', unit: 'per-ton' })

// Review estimated mineral reserves
const reserves = await mine.getReserves({ siteId: 'florida-phosphate-matrix', mineralType: 'phosphate' })
