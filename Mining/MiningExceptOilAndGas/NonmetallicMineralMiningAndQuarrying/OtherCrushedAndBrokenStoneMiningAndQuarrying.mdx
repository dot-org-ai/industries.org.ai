...
```typescript
import { quarryCrushedStone } from '@headlessly/quarry-crushed-stone'

const quarry = quarryCrushedStone()

// Check current inventory by product type
const inventory = await quarry.getInventory({ stoneType: 'basalt', sizeRange: 'rip-rap' })

// Get current specialty aggregate prices
const prices = await quarry.checkPrices({ product: 'traprock-aggregate', region: 'northeast' })
