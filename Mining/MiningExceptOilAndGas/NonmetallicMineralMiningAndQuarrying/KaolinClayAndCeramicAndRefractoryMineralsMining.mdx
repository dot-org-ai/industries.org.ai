...
// Check brightness test results for current inventory
const brightness = await mine.getBrightness({ grade: 'paper-coating', minGEBrightness: 88 })

// Get current kaolin prices by grade
const prices = await mine.checkPrices({ grade: 'premium-coating', market: 'paper' })
