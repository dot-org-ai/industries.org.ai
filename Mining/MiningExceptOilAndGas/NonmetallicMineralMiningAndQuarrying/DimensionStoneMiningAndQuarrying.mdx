...
// Check block inventory by stone variety
const inventory = await quarry.getInventory({ stoneType: 'marble', grade: 'premium' })

// Get current market prices for dimension stone
const prices = await quarry.checkPrices({ variety: 'white-marble', unit: 'per-cubic-foot' })
