...
| CompressorOperator | Maintains and operates field gas compressors for pipeline delivery |
...
// Check current natural gas prices
const prices = await operation.checkPrices({ hub: 'henry-hub', includeFutures: true })

// Review active leases nearing expiration
const expiringLeases = await operation.getLeases({ expiringWithinDays: 90, status: 'active' })
