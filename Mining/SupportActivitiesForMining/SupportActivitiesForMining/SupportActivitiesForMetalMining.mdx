...
```typescript
import { serviceMetalMiningOperations } from '@headlessly/service-metal-mining-operations'

const metalSupport = serviceMetalMiningOperations()

// Check rig availability for upcoming program
const rigs = await metalSupport.getDrillRigs({ type: 'diamond', status: 'available' })

// Review lab turnaround for assay planning
const labStatus = await metalSupport.getLabCapacity({ laboratory: 'ams-reno', elements: ['Au', 'Ag'] })
