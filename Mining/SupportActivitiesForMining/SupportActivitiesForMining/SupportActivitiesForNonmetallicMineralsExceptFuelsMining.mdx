...
groupCode: 2131
action: Service
object: Nonmetallic Mining Operations
---
...
| ShotFirer | Loads blast holes, wires detonators, and fires shots per the blasting engineer's plan |
...
```typescript
import { serviceNonmetallicMiningOperations } from '@headlessly/service-nonmetallic-mining-operations'

const mineralSupport = serviceNonmetallicMiningOperations()

// Check available drill rigs and equipment
const rigs = await mineralSupport.getDrillRigs({ type: 'rotary', status: 'available' })

// Find open service contracts
const contracts = await mineralSupport.findContracts({ mineralType: 'limestone', region: 'midwest' })
