...
```typescript
import { drillOilGasWells } from '@headlessly/drill-oil-gas-wells'

const driller = drillOilGasWells()

// Check current day rates for rig class
const rates = await driller.getDayRates({ rigClass: '1500hp-ac', basin: 'permian' })

// Find available rigs for the job
const rigs = await driller.findRigs({ type: 'ac-vfd', horsePower: 1500, status: 'available' })
