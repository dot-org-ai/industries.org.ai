...
```typescript
import { serviceCoalMiningOperations } from '@headlessly/service-coal-mining-operations'

const coalSupport = serviceCoalMiningOperations()

// Check available drilling rigs
const rigs = await coalSupport.getDrillRigs({ type: 'core-rig', status: 'available' })

// Find open service contracts in the region
const contracts = await coalSupport.findContracts({ region: 'appalachian', services: ['core-drilling'] })
