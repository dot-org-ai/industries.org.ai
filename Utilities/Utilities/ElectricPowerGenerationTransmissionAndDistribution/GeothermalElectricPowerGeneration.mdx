...
// Retrieve production and injection well conditions
const getWellStatusResult = await plant.getWellStatus({ status: 'active' })

// Get pressure, temperature, and chemistry readings
const getReservoirDataResult = await plant.getReservoirData({ status: 'active' })
