...
| spotPriceChanged | Current electricity spot market price retrieved |
...
graph LR
    ...
    P -->|engages| CommunityStakeholder
```
...
// Retrieve current and historical water levels
const getReservoirLevelsResult = await plant.getReservoirLevels({ status: 'active' })

// Get operational status of all turbine units
const getTurbineStatusResult = await plant.getTurbineStatus({ status: 'active' })
...
plant.spotPriceChanged(async ({ price }) => {
