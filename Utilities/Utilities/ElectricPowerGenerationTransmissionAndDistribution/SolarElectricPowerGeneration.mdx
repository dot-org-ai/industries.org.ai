...
| inverterWentOnline | Inverter began converting DC to AC power |
...
// Retrieve current and historical solar radiation
const getIrradianceDataResult = await plant.getIrradianceData({ siteId: 'desert-sunlight', period: 'today' })

// Get operational status of panel arrays
const getArrayStatusResult = await plant.getArrayStatus({ siteId: 'desert-sunlight', status: 'online' })
