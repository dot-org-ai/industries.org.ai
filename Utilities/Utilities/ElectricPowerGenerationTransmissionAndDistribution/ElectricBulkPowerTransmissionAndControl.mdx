...
// Retrieve real-time voltage and flow data
const getGridStatusResult = await transmission.getGridStatus({ region: 'service-area-1' })

// Check energization state of transmission lines
const getLineStatusResult = await transmission.getLineStatus({ status: 'active' })
