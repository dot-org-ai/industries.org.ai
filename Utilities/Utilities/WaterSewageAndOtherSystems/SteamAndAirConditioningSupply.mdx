...
| SteamTrapInspector | Tests steam traps throughout the distribution network to identify failures wasting condensate |
...
graph LR
    ...
    D -->|coordinates with| CityPlanning
```
...
// Retrieve boiler and chiller operational status
const getPlantStatusResult = await district.getPlantStatus({ status: 'active' })

// Check pressure in steam and chilled water mains
const getDistributionPressureResult = await district.getDistributionPressure({ status: 'active' })
