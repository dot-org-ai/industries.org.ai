...
| CorrosionTechnician | Reads cathodic protection test stations and maintains pipeline coatings to prevent corrosion |
...
| coldWeatherWarningIssued | Cold weather warning issued affecting distribution |
| supplyBecameConstrained | Gas availability limited at city gate |
...
// Retrieve customer service and billing information
const getCustomerAccountResult = await utility.getCustomerAccount({ status: 'active' })

// Get consumption data for specific meter
const getMeterReadingResult = await utility.getMeterReading({ status: 'active' })
...
utility.coldWeatherWarningIssued(async ({ forecast, duration, severity }) => {
  if (severity === 'extreme') {
