...
| SecuritiesRegulator | Securities and Exchange Commission regulating securities exchanges |
...
graph LR
    ...
    Exchange -->|regulated by| SecuritiesRegulator
...
// Check current market statistics
const stats = await exchange.getMarketStatistics({ period: 'today' })

// Check for any halted securities
const halted = await exchange.findHaltedSecurities({ status: 'active' })
