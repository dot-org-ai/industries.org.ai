...
```typescript
import { administerWelfareFunds } from '@headlessly/administer-welfare-funds'

const welfareFund = administerWelfareFunds()

// Check open enrollment status
const enrollmentStatus = await welfareFund.getOpenEnrollmentStatus({ planYear: 2024 })

// Review COBRA participants
const cobraMembers = await welfareFund.getCOBRAParticipants({ status: 'active' })
