...
```typescript
import { administerPensions } from '@headlessly/administer-pensions'

const pensionFund = administerPensions()

// Check plan funding status
const funding = await pensionFund.getFundingStatus({ planId: 'db-plan-001' })

// Review current asset allocation
const allocation = await pensionFund.getAssetAllocation({ planId: 'db-plan-001' })
