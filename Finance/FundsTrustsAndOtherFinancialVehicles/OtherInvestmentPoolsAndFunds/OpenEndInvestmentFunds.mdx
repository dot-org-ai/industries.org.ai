...
| SecuritiesRegulator | Securities and Exchange Commission regulating funds |
...
| NAVAccountant | Specialist calculating daily net asset values and fund pricing |
...
graph LR
    ...
    Fund -->|regulated by| SecuritiesRegulator
...
// Check pending transactions before NAV calculation
const pending = await mutualFund.getPendingTransactions({ fundId: 'growth-fund-001' })

// Review current portfolio holdings
const holdings = await mutualFund.getPortfolioHoldings({ fundId: 'growth-fund-001' })
