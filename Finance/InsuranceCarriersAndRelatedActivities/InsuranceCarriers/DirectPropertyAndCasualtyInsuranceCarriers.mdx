...
// Check catastrophe exposure for region
const exposure = await carrier.getCatastropheExposure({ region: 'ST-12345', peril: 'windstorm' })
...
// Check claims pipeline before reporting new claim
const openClaims = await carrier.getClaimsPipeline({ policyId: policy.id, status: 'open' })
