...
```typescript
import { reinsureRisks } from '@headlessly/reinsure-risks'

const reinsurer = reinsureRisks()

// Review existing treaties with this cedant
const treaties = await reinsurer.findTreaties({ cedantId: 'cedant-12345', status: 'active' })

// Check catastrophe exposure before new treaty
const catExposure = await reinsurer.getCatastropheExposure({ peril: 'hurricane', region: 'southeast' })
