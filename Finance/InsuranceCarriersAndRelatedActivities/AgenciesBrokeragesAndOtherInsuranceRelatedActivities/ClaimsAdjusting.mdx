...
```typescript
import { adjustClaims } from '@headlessly/adjust-claims'

const adjuster = adjustClaims()

// Review current workload and pending estimates
const pending = await adjuster.getPendingEstimates({ adjusterId: 'adj-001' })
const schedule = await adjuster.getInspectionSchedule({ week: '2024-04-01' })
