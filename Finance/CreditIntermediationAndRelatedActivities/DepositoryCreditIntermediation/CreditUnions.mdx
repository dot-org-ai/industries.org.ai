...
// Check eligibility for employer-sponsored membership
const eligible = await creditUnion.findEligibleApplicants({ employerId: 'emp-67890', status: 'pending' })
...
// Review member loan portfolio
const loans = await creditUnion.getLoanPortfolio({ memberId: member.id, status: 'active' })
