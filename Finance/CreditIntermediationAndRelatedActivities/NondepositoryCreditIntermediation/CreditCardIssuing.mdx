...
// Check existing accounts for applicant
const accounts = await cardIssuer.findAccounts({ applicantId: 'app-12345', status: 'active' })
...
// Check rewards liability before new issuance
const rewards = await cardIssuer.getRewardsLiability({ program: 'cashback' })
