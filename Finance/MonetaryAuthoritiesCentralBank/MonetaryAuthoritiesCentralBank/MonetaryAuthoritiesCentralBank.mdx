...
| BankExaminer | Field professional conducting on-site regulatory examinations |
...
// Review recent monetary policy decisions and inflation data
const decisions = await centralBank.getMonetaryPolicyDecisions({ period: '2024', limit: 5 })
const inflation = await centralBank.getInflationData({ index: 'CPI', months: 12 })
